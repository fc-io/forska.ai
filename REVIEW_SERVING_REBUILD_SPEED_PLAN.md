...
Status: PR #108's safe implementation slices are complete, and the follow-up rebuild-speed slices removed claimless article-range `judgmentInputContent` rebuild JS materialization, selected-import article-range rebuild JS materialization, selected-import rebuild patch production/reads in rebuild sources, requestless ranged summary quarantine, full summary contribution-state publication, and the remaining unbatched status/posting worker paths. The remaining roadmap should prefer durable direct snapshot/reduction designs over deferring risky rewrites solely for caution.
...
- Still pending: dirty incremental patch support has not been removed; that is a separate direct-table replacement from the missing-snapshot full-rebuild path. Requestless ranged summary chunks no longer keep the legacy path; normal ranged full-summary work is admitted through a rebuild request so finalization owns partial reduction.
...
- Pending: reader/transform worker pool and controlled multi-writer execution.
