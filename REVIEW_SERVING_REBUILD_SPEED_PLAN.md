...
15. Legacy V4 runtime patch-table cleanup, retention drains, and promotion compaction hooks have been removed. Runtime projectors and retention stay off legacy patch/contribution tables, while active serving/base cleanup remains protected by active, last-known-good, and pinned snapshots.
...
Status: complete for production-safe measured hotspots.
...
- Implemented: dirty incremental legacy patch support has been removed from runtime projectors. Dirty projectors now update direct serving/base/detail tables while retaining acknowledgement, manifest, watermark, and active/candidate semantics; runtime retention now only cleans current serving/base tables.
...
Status: complete for production-safe single-writer rebuild execution; true multi-writer is paused until further notice.
...
- Paused for production rebuild chunks: controlled multi-writer execution is not active work. It needs a new explicit decision plus component-specific live benchmark evidence, write-conflict coverage, and memory-spill coverage before any production rebuild chunk uses it. Until then, rebuild chunks stay on serialized multi-range writer transactions.
