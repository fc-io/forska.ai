...
Implemented prerequisite: request-associated chunked full summary rebuilds now stage range partials and reduce them during completed-request finalization, with tests covering partial writes and reducer SQL. Fresh missing-snapshot LLM and human status chunks now avoid status patch rows and use direct serving updates. Selected-import article-range rebuild chunks now write base rows directly with SQL-native statements. Remaining prerequisite: extend the direct no-patch/final-table cutover to any selected-import patch-producing rebuild shape still admitted outside the fresh bootstrap path, or remove that shape entirely after validating callers. Blocker: non-fresh selected-import rebuild chunks still intentionally project selected-import patch rows against active/candidate snapshots, and removing that path requires proving no incremental active/candidate patch replay callers still depend on it.
...
- Implemented: selected-import article-range rebuild chunks now delete/insert scoped `app.review_selected_article_import_v4` rows with SQL-native statements and use a post-write count, while the cursor-based selected-import batch path remains unchanged for full non-ranged drains.
