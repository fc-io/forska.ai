...
- Implemented: `runReviewServingProjectorWorkerCycle` supports a configurable `rebuildChunkBatchSize` for claiming/running several rebuild chunks in one wake. The maintenance heartbeat wires it to `FORSKA_REVIEW_SERVING_REBUILD_CHUNK_BATCH_SIZE`, defaulting to 2. `FORSKA_REVIEW_SERVING_REBUILD_CHUNK_BATCH_MAX_RSS_BYTES` defaults to an auto system-memory-derived cap and limits multi-chunk batching back to one chunk once process RSS reaches the cap; explicit env values can still force size 1 or cap 0/off. Background multi-chunk batches now preclaim only same-request, same-component, same-projection, range-disjoint chunks before serial execution; foreground/request chunks keep single-claim behavior so reader-yield semantics stay intact. Compatible ranged fresh `projectScope` chunks and ranged `selectedImport`, `display`, `payload`, `search`, `queue`, and `judgmentInputContent` chunks now use batch writers that submit all preclaimed ranges through one serialized projector-writer transaction before validating/completing each manifest.
- Pending: reader/transform worker pool, more component-specific set-based multi-chunk writers, and controlled multi-writer execution.
