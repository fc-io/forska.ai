...
## Current Decision - True Multi-Writer Rebuild Writes

True multi-writer rebuild writes are not ready for production wiring. The production path remains the current serialized multi-range writer under the maintenance profile. `runDuckdbAppendTransaction` and `FORSKA_DUCKDB_APPEND_TRANSACTION_ENABLED=true` are experiment-only primitives and must not be wired into production rebuild chunks yet.

Production wiring requires all of these gates to pass for the specific component/table/range shape being enabled:
...
15. Legacy V4 runtime patch-table cleanup and promotion compaction hooks have been removed from selected-import rebuild reset and snapshot promotion. Runtime projectors stay off legacy patch/contribution tables, while retention keeps bounded legacy patch/contribution drains as an operator cleanup exception for old rows alongside active serving/base cleanup protected by active, last-known-good, and pinned snapshots.
...
- Implemented: dirty incremental legacy patch support has been removed from runtime projectors. Dirty projectors now update direct serving/base/detail tables while retaining acknowledgement, manifest, watermark, and active/candidate semantics; retention still runs bounded legacy patch/contribution drains only as an operator cleanup exception for old rows.
...
Status: complete for production-safe single-writer rebuild execution; true multi-writer is future/benchmark-gated.
