...
| Phase 6 - Controlled Parallelism | Partially implemented | The worker now supports an opt-in `rebuildChunkBatchSize` that can claim and execute multiple rebuild chunks in one wake while preserving the existing serialized writer lane. True read/transform parallelism, set-based multi-chunk write, and controlled multi-writer execution are still pending. |
...
- Implemented: generic projector writes report input/deduped record counts, batch counts, and write ms by table. Posting rebuild chunks propagate source-query, diff-input transform, contribution-diff, record-transform, stats, delete-build, and writer timing diagnostics into chunk `diagnostics_json`.
...
- Implemented: full posting rebuild chunks skip `mart.review_article_filter_posting_patch_v4` writes and write only serving/stat/contribution state needed by the final candidate snapshot and future incremental diffs.
...
- Implemented: `runReviewServingProjectorWorkerCycle` supports an opt-in `rebuildChunkBatchSize` for claiming/running several rebuild chunks in one wake. The batch still executes serially through the existing writer lane, which reduces claim/cycle overhead without introducing concurrent DuckDB writes.
