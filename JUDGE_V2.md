...
## Postgres / Drizzle
- [ ] `src/db/schema.ts`: add 4 bool cols on `judgments` (NOT NULL); defaults: `useTitle=true,useAbstract=true,useFulltext=false,useFulltextNoImages=false`
- [ ] DB backfill: set legacy rows to same defaults (avoid old rows becoming all-false)
- [ ] Add index `(article_id,prompt_id,model_id,useTitle,useAbstract,useFulltext,useFulltextNoImages)`; if unique, include soft-delete (`deletedAt`) or partial-unique on `deletedAt IS NULL` (so rejudge can insert)
...
- [ ] `src/server/cron/judgmentsJobs/judgmentsJobsCronGetPrompts.ts`: NOT EXISTS match on `(articleId,promptId,project.modelId,useTitle,useAbstract,useFulltext,useFulltextNoImages,isAnswered=true,deletedAt IS NULL)`
