...
- Run `bun test src/db/migrateDuckdb.workflowTables.test.ts src/db/migrateDuckdb.conflictResolutionTables.test.ts src/db/migrateDuckdb.reviewRebuildChunkManifest.test.ts --timeout 120000`
  for old and fresh judgment-workflow databases, comparison conflict-resolution,
  and rebuild chunk-manifest upgrade paths. Existing databases with inline
  `app.judgment_job`, `app.comparison_project_serving_generation`, indexed
  `app.comparison_project_conflict_resolution`, or indexed
  `app.review_rebuild_chunk_manifest` tables must cross the migration boundary
  without triggering startup indexed-table recovery, and the final tables must
  retain rows/defaults while having no primary keys or mutable lookup indexes.
...
Keep the explicit `./` path prefixes for Bun test file arguments in this
DuckDB/runtime section. With Bun 1.3.13, bare relative paths such as
`bun test src/server/utils/duckdbEngineCompatibility.test.ts` can make spawned
child Bun processes exit with empty stdout/stderr, which hides the real JSON
subprocess output these tests assert.
...
bun test scripts/buildPlaywrightApp.test.ts scripts/startPlaywrightApiServer.test.ts scripts/checkReviewServingCurrentDbTabCountParity.test.ts
```

These protect cold readiness without opening DuckDB, background queue draining
across cold startup and recycle, foreground preemption of queued background
transactions, event-driven foreground-idle admission with
...
identity by pinned title/abstract hashes, Playwright current-DB API server
shutdown releasing its DuckDB owner lease when the parent exits, and the
current-DB tab-count parity one-shot script closing its DuckDB owner lease. The projector worker admission checks
...
| `bun test src/server/reviewServing/reviewServingSql.test.ts src/server/reviewServing/reviewServingReader.test.ts src/server/reviewServing/reviewServingLlmReviewRouteService.test.ts --timeout 120000`                                                                                                                                                                                                                                                                                                                                                                                | LLM review serving read/status path                                                      | Covers shared review-serving row SQL shape, reader gates, and LLM review list rows deriving visible completion from serving `llm_status` even when judgment-detail hydration is sparse.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `bun test ./src/server/utils/env.test.ts ./src/server/utils/reviewServingProjectorWorkerHeartbeat.test.ts ./src/server/workers/reviewServingProjectorWorker.test.ts ./src/server/reviewServing/reviewServingProjectorService.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                             | Review-serving worker loop knobs and controlled chunk batching                           | Covers the env/heartbeat memory-aware default `FORSKA_REVIEW_SERVING_REBUILD_CHUNK_BATCH_SIZE` selection, system-memory-bounded `FORSKA_REVIEW_SERVING_REBUILD_CHUNK_BATCH_MAX_RSS_BYTES`, explicit single-chunk/cap override preservation, `FORSKA_DUCKDB_APPEND_TRANSACTION_ENABLED=false` defaulting for the paused multi-writer primitive, heartbeat propagation into the worker loop, compatible requestless range-disjoint background chunk preclaiming, incompatible and summary chunk preclaim rejection, rebuild chunk executors using background-priority transactions when available, fresh project-scope, display, payload, llmStatus, humanStatus, queue, judgment-input-content, and posting SQL-native multi-range batch writing through the serialized projector writer, selected-import foreground rebuilds yielding after one range with tighter range splitting, search dirty work admitting bounded search rebuilds instead of in-place title-search array updates, oversized search rebuild ranges splitting before writer execution, high-RSS maintenance-owner restarts pausing projector recovery before process exit, small foreground search ranges batching in a bounded group while larger/unknown search ranges still yield between chunks, status rebuild chunks staying off crash-prone per-chunk DuckDB recycle/forced-GC cleanup, bounded fallback preparation concurrency with live leases before serialized writes, prepared request finalization, deterministic request adoption for requestless ranged summary chunks with real-DuckDB request/chunk linkage coverage, all-preclaimed-chunk failure handling on batch-writer errors, prepared fallback write failure handling, multi-chunk rebuild batches per worker wake, and RSS-cap fallback to one chunk while keeping writes serialized.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
