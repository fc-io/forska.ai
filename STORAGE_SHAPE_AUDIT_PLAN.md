...
| Field | Value | Reason |
| --- | --- | --- |
| `overallCertification` | `INCOMPLETE` | The API manifest has 22 nonterminal rows and one blocked row; the US-003 schema census adds nonterminal object/field rows plus open-JSON blockers; the remaining manifests, lifecycle proof, benchmark-critical values, and physical proof are pending; and no inherited recommendation is actionable. |
| Framework version | `US-001 / 2026-07-21` | First normalized, resumable evidence structure. |
| Latest normalized story | `US-003 / 2026-07-21` | Article, import, source-record, project-scope, and logical subproject shapes are statically enumerated from the effective forward migration chain; deployed/runtime evidence and closed schemas for open JSON remain unavailable. |
...
| DuckDB/persisted objects | 20 | 0 | 0 | 0 | 20 | `20 != 0 + 0 + 0` | US-003 article/import/project-scope slice: 9 tables, 1 logical no-table shape, and 10 indexes; lifecycle/runtime proof remains |
| Columns/material fields | 155 | 0 | 0 | 9 | 146 | `155 != 0 + 0 + 9` | US-003 slice: 113 physical columns plus 42 JSON key/path-family rows; nine open JSON domains blocked |
| Indexes (DBO subset) | 10 | 0 | 0 | 0 | 10 | `10 != 0 + 0 + 0` | Declared current indexes enumerated; deployed existence, optimizer use, size, and write cost unavailable |
...
US-003 schema slice adds 20 nonterminal object rows and reconciles 155 material
fields as 146 nonterminal plus nine blocked open-JSON rows. Later census stories
must append rather than renumber these rows and update affected counts.
...
| `OQ-0010` | Database/audit owner | Which immutable non-live snapshot may certify the applied migration set, exact deployed columns/constraints/indexes, optimizer plans, row/byte/distribution/timestamp evidence, and absence of dynamic/generated references for this census? | `BLK-0007`, `BLK-0009`, all nonterminal US-003 rows, and physical/index proof |
