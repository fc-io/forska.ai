...
- [ ] Change ReplacingMergeTree version column from `createdAt` to `updatedAt`
  ...
  - Requires ALTER TABLE or table recreation
...
- [ ] Create single endpoint `POST /api/admin/sync-judgments-to-clickhouse` that:
  1. Syncs new judgments (current backfill logic)
  2. Syncs updated judgments (where `updatedAt > createdAt`)
  3. Syncs deleted judgments (tombstones)
...
-- Recreate with updatedAt as version (requires data migration)
-- Option A: Create new table, migrate data, rename
-- Option B: Accept current data uses createdAt, new data uses updatedAt
```
