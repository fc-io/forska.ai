...
- [ ] Remove obsolete refresh paths and mart tables in the same clean-cut slice that rewrites callers; do not keep compatibility shims or parallel maintenance paths.
...
- [ ] Cutover clearing must be explicit, worker-quiesced, and fenced: acquire the durable exclusive cutover fence, pause refresh/rebuild/queue drains, wait for active workers to stop or leases to expire, remove obsolete write paths, verify obsolete callers/writers cannot recreate legacy rows, keep the durable fence so app/worker/pre-cutover invocations fail closed, clear obsolete intermediate state, rederive replacement dirty or rebuild work from source-of-truth tables under the cutover owner token, release the fence, then resume workers.
- [ ] Never publish ACKs or advance completed tokens merely because obsolete intermediate state was cleared during cutover.
- [ ] Cutover must hold a durable exclusive fence read by maintenance workers, new dirty/rebuild write paths, and any pre-cutover in-flight obsolete queue/refresh invocation: while the fence is active, workers and app write paths must not claim, ACK, complete, or enqueue refresh/rebuild/queue work; only the cutover owner token may recreate replacement dirty-state rows or large-rebuild requests, and obsolete invocations must fail closed instead of accepting rows.
...
- [ ] Update judgment-job unassessed count/article routes to treat incomplete project-wide dirty materialization and unresolved quarantine barriers as not-fully-fresh, using a raw source-of-truth read path or explicit pending state without caching a stale OLAP count as fresh.
...
- [ ] Delete or rename `refreshProject(projectId)` to an explicit structural rebuild/repair API; no generic project refresh API should remain in the long-term target.
- [ ] Add a cutover migration or maintenance step that clears obsolete in-flight dirty, queue, and rebuild intermediate state only after obsolete write paths are removed, verified unable to recreate legacy rows, workers are paused, and the durable cutover fence is held, then immediately recreates dirty-state rows or large-rebuild requests from current source-of-truth tables under the cutover owner token before fence release and worker resume.
- [ ] Add an explicit `db:duck:rebuild2-cutover` package command in the same clean-cut slice that removes legacy state paths: acquire the durable cutover fence, pause workers, wait for active workers to stop or leases to expire, remove obsolete legacy queue/refresh/rebuild write paths, verify legacy queue write paths are removed and stale invocations fail closed, verify no legacy queue callers remain, clear obsolete queue/dirty/rebuild/lease state, rederive replacement dirty or large-rebuild work under the cutover owner token, release the fence, then resume workers.
- [ ] During cutover, also delete stale obsolete `app.maintenance_work_lease` rows for obsolete review-index article refresh, project refresh, archived-project recovery work, and any large-rebuild lease whose backing rebuild state was cleared or rederived; if auditability is required, replace them only with explicit cutover audit/tombstone rows that cannot be interpreted as active, recoverable, ACKed, or completed work.
- [ ] During cutover and judgment-job deletion/start-clean cleanup, delete stale obsolete `judgment_sqlite_outbox_import` rows in `app.maintenance_work_lease` and remove orphaned `app.judgment_job_sqlite_health_projection` rows without owner ACK; if auditability is required, replace them only with explicit cutover audit/tombstone rows that cannot be interpreted as active, retrying, ACKed, or waiting for owner ACK.
...
- [ ] Define and implement the quarantine ACK barrier: quarantined article rows remain durable with quarantine metadata, and project `last_completed_dirty_token`, maintenance claim completion, and any SQLite outbox/import ACK whose success depends on dirty-refresh durability must not advance past the earliest unresolved quarantined dirty token.
- [ ] Rename `app.project_mart_refresh_article_quarantine` to `app.project_mart_dirty_refresh_article_quarantine` in the same quarantine migration slice that changes article-only rows to project/article dirty-token-scoped quarantine rows, with indexes that can find the earliest unresolved quarantined token per project; cut over or clear obsolete article-only quarantine rows while workers are paused and the cutover fence is held, and do not keep the old table name.
...
- [ ] Add explicit unquarantine/retry or bounded cleanup semantics before a quarantined row can be resolved and then included in ACK advancement or pruning.
...
- [ ] Rename large-rebuild state fields from generic refresh-token terminology to rebuild-token or maintenance-token terminology; old names must only appear in migration/rename code.
...
- [ ] Delete or rename `db:duck:inspect-project-refresh-risk` and `db:duck:recover-project-refresh-claims` so operational CLIs use dirty-refresh/dirty-materialization terminology, understand durable dirty-materialization state, dirty-token quarantine barriers, project date-window/curated scope, and large-rebuild leases, and do not keep project-refresh claim terminology.
...
- [ ] Rename `getDuckdbMartRefreshService` and its tests to a mart-maintenance service name after queue deletion, so the long-term service name covers dirty refresh, large rebuild, and bounded cleanup without preserving legacy refresh-queue terminology.
...
- [ ] Assert cutover acquires the durable fence, blocks worker claims/ACKs/completions, removes legacy queue write paths, makes stale queue writes fail closed, verifies no legacy queue callers remain before clearing, clears obsolete intermediate state and deletes or tombstones stale maintenance leases without ACK/completion semantics, rederives replacement work from source-of-truth tables under the cutover owner token, releases the fence, resumes workers, and does not publish ACKs from clearing alone.
...
- [ ] Assert `judgeStoreJudgment` stores the judgment and marks affected projects dirty atomically; if dirty-state marking fails, the judgment write cannot be silently committed as refreshed, and no queue-named wrapper remains around the dirty-state write.
...
- [ ] Assert a claim with quarantined rows processes non-quarantined rows, does not publish ACK or advance `last_completed_dirty_token` past the quarantine barrier, releases or parks without failure when only quarantined rows remain, and resumes after explicit unquarantine/retry or bounded cleanup.
...
- [ ] `bun test scripts/quarantineProjectMartDirtyRefreshArticle.test.ts scripts/unquarantineProjectMartDirtyRefreshArticle.test.ts`
- [ ] `bun run db:mig`
- [ ] `bun run lint`
- [ ] `bun run build`
