...
- Analyze claims queued sessions and owns zip validation, manifest validation, checksum verification, extraction into session temp storage, package parsing, mapping analysis, plan creation, and `planRevision` changes.
...
- Cancellation handles `awaiting_upload`, `uploading`, `queued`, `extracting`, `analyzing`, `awaiting_resolution`, and `ready_to_commit` with writer-owned state transitions and temp cleanup; `committing`, `completed`, and `failed` are rejected, while repeated `cancelled` or `expired` cancellation calls are idempotent.
...
- Update `ProjectTransferPlanSummary.overlapCounts` to expose the final overlap fields: `reusedArticleCount`, `newArticleCount`, `reusedArticleUpdateCount`, `reusedArticleFieldFillCount`, `reusedArticleAssetPromotionCount`, `reusedJudgmentCount`, `dirtiedExistingProjectCount`, `omittedRouteLinkCount`, `omittedArticleRouteLinkCount`, `routeArticleSnapshotLinkCount`, `duplicateImportMatchCount`, `storedSignatureJudgmentCount`, `snapshotVerifiedJudgmentCount`, `currentReviewRowsSignatureJudgmentCount`, `storedSignatureHumanReviewCount`, and `currentReviewRowsSignatureHumanReviewCount`. Do not expose the Phase 1 placeholder names `exactDuplicateImports` and `reusedArticles` unless they are explicitly documented as mapped aliases.
- Update `ProjectTransferPlanSummary.conflictCounts` to expose final blocker/conflict fields such as `packageContractConflictCount`, `articleConflictCount`, `projectPromptConflictCount`, `judgmentConflictCount`, `humanReviewFidelityConflictCount`, and dependency/model unresolved counts. Do not expose the Phase 1 placeholder conflict names unless they are explicitly documented as mapped aliases.
...
- Treat the wizard-visible judgment conflict status as unresolved/unknown while required model mappings are unresolved; because the current `ProjectTransferConflictCounts` contract only accepts non-negative integers, represent that state outside `conflictCounts` or intentionally widen the contract and update `projectTransferContracts.test.ts`. Require a concrete `judgmentConflictCount` before entering `ready_to_commit`.
