CREATE TEMP TABLE project_child_parent_fk_comparison_project_backup AS
SELECT * FROM app.comparison_project;

CREATE TEMP TABLE project_child_parent_fk_comparison_project_source_project_backup AS
SELECT * FROM app.comparison_project_source_project;

CREATE TEMP TABLE project_child_parent_fk_judgment_backup AS
SELECT * FROM app.judgment;

CREATE TEMP TABLE project_child_parent_fk_judgment_assessment_backup AS
SELECT * FROM app.judgment_assessment;

CREATE TEMP TABLE project_child_parent_fk_judgment_human_backup AS
SELECT * FROM app.judgment_human;

CREATE TEMP TABLE project_child_parent_fk_judgment_human_summary_backup AS
SELECT * FROM app.judgment_human_summary;

CREATE TEMP TABLE project_child_parent_fk_judgment_job_backup AS
SELECT * FROM app.judgment_job;

CREATE TEMP TABLE project_child_parent_fk_project_article_backup AS
SELECT * FROM app.project_article;

CREATE TEMP TABLE project_child_parent_fk_project_import_route_backup AS
SELECT * FROM app.project_import_route;

CREATE TEMP TABLE project_child_parent_fk_project_mart_dirty_materialization_state_backup AS
SELECT * FROM app.project_mart_dirty_materialization_state;

CREATE TEMP TABLE project_child_parent_fk_project_mart_dirty_refresh_article_quarantine_backup AS
SELECT * FROM app.project_mart_dirty_refresh_article_quarantine;

CREATE TEMP TABLE project_child_parent_fk_project_mart_large_rebuild_state_backup AS
SELECT * FROM app.project_mart_large_rebuild_state;

CREATE TEMP TABLE project_child_parent_fk_project_mart_refresh_article_state_backup AS
SELECT * FROM app.project_mart_refresh_article_state;

CREATE TEMP TABLE project_child_parent_fk_project_mart_refresh_state_backup AS
SELECT * FROM app.project_mart_refresh_state;

CREATE TEMP TABLE project_child_parent_fk_project_prompt_backup AS
SELECT * FROM app.project_prompt;

CREATE TEMP TABLE project_child_parent_fk_review_backup AS
...
DROP INDEX IF EXISTS app.idx_app_project_mart_dirty_materialization_state_claim;
DROP INDEX IF EXISTS app.idx_app_project_mart_dirty_materialization_state_project_token;
DROP INDEX IF EXISTS app.idx_app_project_mart_dirty_materialization_state_stale_work;
...
DROP INDEX IF EXISTS app.idx_app_project_mart_large_rebuild_state_claim;
DROP INDEX IF EXISTS app.idx_app_project_mart_large_rebuild_state_current;
DROP INDEX IF EXISTS app.idx_app_project_mart_large_rebuild_state_stale_work;
...
DROP TABLE app.project_mart_dirty_materialization_state;
...
CREATE TABLE app.comparison_project (
  ...
  allow_conflict_resolution BOOLEAN NOT NULL DEFAULT FALSE,
  human_judgment_mode human_judgment_mode DEFAULT 'prompt',
  summary_source_project_id VARCHAR,
...
CREATE TABLE app.project_article (
  ...
  project_id VARCHAR NOT NULL,
  article_id VARCHAR NOT NULL REFERENCES app.article(id),
  imported_from_project_id VARCHAR,
  created_at TIMESTAMPTZ NOT NULL DEFAULT current_timestamp,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT current_timestamp,
  UNIQUE(project_id, article_id)
);
...
CREATE TABLE app.project_mart_large_rebuild_state (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT current_timestamp,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT current_timestamp,
  source_dirty_token BIGINT,
  source_high_water_dirty_token BIGINT,
...
INSERT INTO app.comparison_project BY NAME
SELECT * FROM project_child_parent_fk_comparison_project_backup;

INSERT INTO app.comparison_project_source_project BY NAME
SELECT * FROM project_child_parent_fk_comparison_project_source_project_backup;

INSERT INTO app.judgment BY NAME
SELECT * FROM project_child_parent_fk_judgment_backup;

INSERT INTO app.judgment_assessment BY NAME
SELECT * FROM project_child_parent_fk_judgment_assessment_backup;

INSERT INTO app.judgment_human BY NAME
SELECT * FROM project_child_parent_fk_judgment_human_backup;

INSERT INTO app.judgment_human_summary BY NAME
SELECT * FROM project_child_parent_fk_judgment_human_summary_backup;

INSERT INTO app.judgment_job BY NAME
SELECT * FROM project_child_parent_fk_judgment_job_backup;

INSERT INTO app.project_article BY NAME
SELECT * FROM project_child_parent_fk_project_article_backup;

INSERT INTO app.project_import_route BY NAME
SELECT * FROM project_child_parent_fk_project_import_route_backup;

INSERT INTO app.project_mart_dirty_materialization_state BY NAME
SELECT * FROM project_child_parent_fk_project_mart_dirty_materialization_state_backup;

INSERT INTO app.project_mart_dirty_refresh_article_quarantine BY NAME
SELECT * FROM project_child_parent_fk_project_mart_dirty_refresh_article_quarantine_backup;

INSERT INTO app.project_mart_large_rebuild_state BY NAME
SELECT * FROM project_child_parent_fk_project_mart_large_rebuild_state_backup;

INSERT INTO app.project_mart_refresh_article_state BY NAME
SELECT * FROM project_child_parent_fk_project_mart_refresh_article_state_backup;

INSERT INTO app.project_mart_refresh_state BY NAME
SELECT * FROM project_child_parent_fk_project_mart_refresh_state_backup;

INSERT INTO app.project_prompt BY NAME
SELECT * FROM project_child_parent_fk_project_prompt_backup;

INSERT INTO app.review BY NAME
SELECT * FROM project_child_parent_fk_review_backup;

DROP TABLE project_child_parent_fk_comparison_project_backup;
DROP TABLE project_child_parent_fk_comparison_project_source_project_backup;
DROP TABLE project_child_parent_fk_judgment_backup;
DROP TABLE project_child_parent_fk_judgment_assessment_backup;
DROP TABLE project_child_parent_fk_judgment_human_backup;
DROP TABLE project_child_parent_fk_judgment_human_summary_backup;
DROP TABLE project_child_parent_fk_judgment_job_backup;
DROP TABLE project_child_parent_fk_project_article_backup;
DROP TABLE project_child_parent_fk_project_import_route_backup;
DROP TABLE project_child_parent_fk_project_mart_dirty_materialization_state_backup;
DROP TABLE project_child_parent_fk_project_mart_dirty_refresh_article_quarantine_backup;
DROP TABLE project_child_parent_fk_project_mart_large_rebuild_state_backup;
DROP TABLE project_child_parent_fk_project_mart_refresh_article_state_backup;
DROP TABLE project_child_parent_fk_project_mart_refresh_state_backup;
DROP TABLE project_child_parent_fk_project_prompt_backup;
DROP TABLE project_child_parent_fk_review_backup;
...
CREATE INDEX IF NOT EXISTS idx_app_project_mart_dirty_materialization_state_claim ON app.project_mart_dirty_materialization_state(materialization_status, source_kind, target_dirty_token);
CREATE INDEX IF NOT EXISTS idx_app_project_mart_dirty_materialization_state_stale_work ON app.project_mart_dirty_materialization_state(materialization_status, lease_expires_at);
CREATE INDEX IF NOT EXISTS idx_app_project_mart_dirty_materialization_state_project_token ON app.project_mart_dirty_materialization_state(project_id, target_dirty_token, materialization_status);
...
CREATE INDEX IF NOT EXISTS idx_app_project_mart_large_rebuild_state_claim ON app.project_mart_large_rebuild_state(refresh_status, refresh_token, rebuild_phase);
CREATE INDEX IF NOT EXISTS idx_app_project_mart_large_rebuild_state_stale_work ON app.project_mart_large_rebuild_state(refresh_status, lease_expires_at);
CREATE INDEX IF NOT EXISTS idx_app_project_mart_large_rebuild_state_current ON app.project_mart_large_rebuild_state(project_id, refresh_token, target_generation, superseded_at);
...
CREATE INDEX IF NOT EXISTS idx_app_project_mart_refresh_state_claim ON app.project_mart_refresh_state(refresh_status, dirty_token, last_completed_dirty_token);
CREATE INDEX IF NOT EXISTS idx_app_project_mart_refresh_state_stale_work ON app.project_mart_refresh_state(refresh_status, lease_expires_at);
