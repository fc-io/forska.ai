DROP TABLE IF EXISTS mart.review_article_judgment_detail_serving_v4_human_scalars_next;
...
INSERT INTO mart.review_article_judgment_detail_serving_v4_human_scalars_next (
  project_id,
  review_config_hash,
  snapshot_id,
  list_mode_key,
...
SELECT
  ...
  prompt_type,
  prompt_criteria_disposition,
  TRY_CAST(json_extract_string(judgment_payload_json, '$.createdAt') AS TIMESTAMPTZ) AS judgment_created_at,
  CASE
    WHEN payload_kind = 'human' THEN json_extract_string(judgment_payload_json, '$.comment')
    ELSE NULL
  END AS human_comment,
  CASE
    WHEN payload_kind = 'human' OR placeholder_kind IS NOT NULL THEN NULL
    ELSE judgment_payload_json
  END AS judgment_payload_json,
  placeholder_kind,
  detail_updated_at
...
ALTER TABLE mart.review_article_judgment_detail_serving_v4_human_scalars_next
