...
export const europePmcPprWorkflowStoreEntries = async (entries: DatabaseEntry[]): Promise<void> => {
  const validated = entries.map((e, i) => {
    try {
      return DatabaseItem.assert(e)
    } catch (err) {
      throw new Error(`Validation failed for Europe PMC PPR entry ${i}: ${String(err)}`, {cause: err})
    }
  })

  const batches = batchEntries(validated, 500)
  globalThis.console.log(`Storing ${validated.length} Europe PMC PPR records in ${batches.length} batches`)

  for (let i = 0; i < batches.length; i++) {
    const batch = batches[i]
    if (!batch) continue
    globalThis.console.log(`Storing Europe PMC PPR batch ${i + 1}/${batches.length} (${batch.length} records)`)
  ...
  globalThis.console.log(`Successfully stored ${validated.length} Europe PMC PPR records to server`)
}
