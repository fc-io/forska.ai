...
export const fetchTokenUseLast10Minutes = async (): Promise<string> => {
  try {
    ...
    const response = await apiClient.api.tokens.get({
      query: {startTime: tenMinutesAgo.toISOString(), endTime: now.toISOString()},
    })

    if (response.error || response.data?.error) {
      console.error('Error fetching last 10 minutes token use:', response.error || response.data?.error)
      return ''
    }
