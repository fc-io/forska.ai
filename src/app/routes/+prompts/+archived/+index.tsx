    ...
    <div class="min-h-screen bg-gray-50 p-6 mx-auto">
      <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-4">
          <Button as={Link} to="/prompts" variant="outline" size="sm">
            ← Back to Prompts
          </Button>
          <h1 class="text-2xl font-bold">Archived Prompts</h1>
        </div>
      </div>
      ...
      <Show when={!promptsQuery.isLoading} fallback={<div class="text-center py-8">Loading archived prompts...</div>}>
        <Show when={promptsQuery.isError}>
          <div class="text-center py-8 text-red-600">
            Error loading archived prompts:{' '}
        ...
        <Show when={!promptsQuery.isError && (promptsQuery.data?.length ?? 0) === 0}>
          <div class="text-center py-12">
            <h2 class="text-xl font-semibold mb-4">No archived prompts</h2>
            <p class="text-muted-foreground mb-6">Prompts you archive will show up here.</p>
            <Button as={Link} to="/prompts">
              Back to Prompts
            </Button>
          </div>
        </Show>
                  ...
                  <div class="border rounded-lg p-4 bg-background">
                    <div class="flex justify-between items-start gap-4">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (!confirm('Unarchive this prompt? It will return to the active prompts list.')) {
                            return
                          }
                      ...
                      >
                        {pendingPromptId() === prompt.id ? 'Restoring...' : 'Unarchive'}
                      </Button>
                    </div>
...
export const Route = createFileRoute('/prompts/archived/')({component: ArchivedPromptsPage})
