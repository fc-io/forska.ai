...
export const projectsRoutes = new Elysia()
  ...
  .patch(
    '/api/projects/:id/prompts/:promptId/archive',
    async ({params, body}) => {
      const db = getDatabase()
      ...
      const updated = await db
        ...
        .where(and(eq(projectPrompts.projectId, projectId), eq(projectPrompts.promptId, promptId)))
        .returning({
      ...
      if (!archived) {
        return {data: {projectId, promptId, archived, enabled: false, originProjectId: null, order: null}}
      }

      const [maxOrderRow] = await db
        .select({maxOrder: sql<number>`COALESCE(MAX(${projectPrompts.order}), 0)`})
        .from(projectPrompts)
        .where(eq(projectPrompts.projectId, projectId))

      const orderVal = Number(maxOrderRow?.maxOrder ?? 0) + 1
      ...
      const [inserted] = await db
        ...
        .onConflictDoUpdate({
          target: [projectPrompts.projectId, projectPrompts.promptId],
          set: {archived: sql`EXCLUDED.archived`, updatedAt: sql`CURRENT_TIMESTAMP`},
        })
        .returning({
          projectId: projectPrompts.projectId,
          promptId: projectPrompts.promptId,
