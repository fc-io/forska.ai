...
const conflictResolutionImportTargetRows = [
  {
    articleId: 'article-1',
    doi: '10.1000/import-match',
    externalArticleId: 'source-ext-1',
    identifierKeys: ['doi\u001F10.1000/import-match', 'doi\u001F10.1000/ambiguous-import'],
    title: 'Article 1',
  },
  {
    articleId: 'article-2',
    ...
    identifierKeys: ['doi\u001F10.1000/import-solo', 'doi\u001F10.1000/ambiguous-import'],
    title: 'Article 2',
  },
]

const getMockConflictResolutionImportIdentifierTargetRows = (statement: string) => {
  return conflictResolutionImportTargetRows.filter((row) => {
...
const getMockConflictResolutionImportIdTitleTargetRows = (statement: string) => {
  return conflictResolutionImportTargetRows.filter((row) => {
    return statement.includes(row.externalArticleId)
  })
}

const getMockConflictResolutionImportArticleIdTargetRows = (statement: string) => {
  return conflictResolutionImportTargetRows.filter((row) => {
...
const getMockConflictResolutionExistingTargetResolutionRows = (
  ...
  return state.conflictResolutionRows
    .filter((row) => {
      return (
        (row.comparisonProjectId ?? 'comparison-project-1') === comparisonProjectId
...
const queryJson = async (
  ...
  if (
    ...
    && statement.includes('article_id IN')
  ) {
    return getMockConflictResolutionExistingTargetResolutionRows(statement, state)
  }
  ...
  if (statement.includes('WITH') && statement.includes('target_identifier.kind ||')) {
    return getMockConflictResolutionImportIdentifierTargetRows(statement)
  }

  if (statement.includes('a.id AS articleId') && statement.includes('a.id IN')) {
    return getMockConflictResolutionImportArticleIdTargetRows(statement)
  }
  ...
  if (statement.includes('regexp_replace(LOWER(TRIM(COALESCE(a.article_title')) {
    return getMockConflictResolutionImportIdTitleTargetRows(statement)
  }
...
test('comparison project create-from-project imports selected conflict resolutions', async () => {
  ...
  const identifierTargetQuery = state.queryStatements.find((statement) => {
    return statement.includes('target_identifier.kind ||')
  })
  ...
  expect(identifierTargetQuery ?? '').toContain("'doi\u001F10.1000/import-match'")
})
...
test('comparison project conflict resolution import analyze returns row details without creating upload session', async () => {
  ...
  const {comparisonProjectsRoutes} = await loadComparisonProjectsRoutes()
  const app = new Elysia().use(comparisonProjectsRoutes)
  const response = await postComparisonProjectConflictResolutionImportAnalyze(app, {
    ...
    rows: [
      ...
      },
      {
        ...
        doi: '10.1000/import-solo',
        identifiers: [],
        resolution: {mode: 'prompt', value: 'prompt-1', label: 'Prompt 1'},
      },
      {
      ...
      },
      {
        ...
        doi: '10.1000/import-solo',
        identifiers: [],
        resolution: {mode: 'summary', value: 'unclear', label: 'Unclear'},
      },
    ],
  })
  const bodyText = await response.text()
  ...
  expect(body.data.importableRows).toMatchObject([
    {
      ...
      sourceArticleRowId: 'source-article-importable',
      sourceExternalArticleId: 'source-ext-2',
      sourceResolutionId: 'source-resolution-importable',
      sourceTitle: 'Importable source',
      targetArticleId: 'article-2',
      targetArticleIds: ['article-2'],
      targetExternalArticleId: 'source-ext-2',
  ...
  expect(body.data.warnings).toMatchObject([{code: 'invalid-target-resolution-value'}])
  expect(state.transactionCalls).toBe(0)
  expect(state.rootRunStatements).toEqual([])
  expect(state.createdComparisonProjectIds).toEqual([])
  expect(state.staleServingIds).toEqual([])
...
test('comparison project conflict resolution import matches same article rows before identifier lookup', async () => {
  ...
  const response = await postComparisonProjectConflictResolutionImportAnalyze(app, {
    ...
    rows: [
      {
        sourceResolutionId: 'source-resolution-article-id',
        sourceArticleRowId: 'article-2',
        externalArticleId: null,
        title: 'Article 2',
        identifiers: [],
  ...
  const body = JSON.parse(bodyText) as {
    data: {importableRows: Array<Record<string, unknown>>; summary: Record<string, number>}
  }
  const state = getMockDatabaseState()
...
test('comparison project conflict resolution import commit can include non-conflicting matched rows', async () => {
  mockDatabaseStateRef.current = {
    ...
    sourceProjectLinks: [{id: 'comparison-project-source-1', sourceProjectId: 'source-project-1'}],
  }
  ...
  const response = await postComparisonProjectConflictResolutionImportCommit(app, {
    artifact: {
      ...
      rows: [
        {
          sourceResolutionId: 'source-resolution-non-conflicting',
          sourceArticleRowId: 'source-article-non-conflicting',
          externalArticleId: 'source-ext-2',
          title: 'Non-conflicting source',
          doi: '10.1000/import-solo',
          identifiers: [],
          resolution: {mode: 'summary', value: 'yes', label: 'Yes'},
        },
  ...
  expect(body.data.skippedRows).toEqual([])
  expect(insertedResolution).toEqual({
    answerValue: 'yes',
    articleId: 'article-2',
    comparisonProjectId: 'comparison-project-1',
...
test('comparison project create-from-project dedupes duplicate conflict resolution import', async () => {
  ...
  const body = JSON.parse(bodyText) as {conflictResolutionImportSummary: ConflictResolutionImportSummaryResponse}
...
test('comparison project export streams ordered csv rows with article context and flattened answers', async () => {
  ...
  expect(csv.trim().split('\n')).toEqual([
    [
      ...
      'Prompt 2 - LLM - Model 1 - Article Title and Abstract',
      'Prompt 2 - LLM - Model 2 - Article Title and Abstract',
      'Prompt 2 - Human answer',
    ].join(','),
    [
      'Article 1',
      'Article 1 summary',
...
test('comparison project export can render a readable pdf with matching filters and reviewer metadata', async () => {
  ...
  expect(pdf).toContain('Export number of articles: 1')
  expect(pdf).not.toContain('Rows in export:')
  expect(pdf).toContain('Offline review')
  expect(pdf).toContain('Terminology')
  expect(pdf).toContain('Review conflict')
  expect(pdf).toContain('An article where AIs, or an AI and human reviewers')
  expect(pdf).toContain('Conflict resolution')
  expect(pdf).toContain('Your final choice for the inclusion of an article')
  expect(pdf).toContain('Summary judgment')
  expect(pdf).toContain('The overall include/exclude choice for the study')
  expect(pdf).toContain('after considering the individual')
  expect(pdf).toContain('criteria.')
  expect(pdf).toContain('LLM assessment')
  expect(pdf).toContain('The model-generated answer and reasoning')
  ...
  expect(pdf).not.toContain('These are the study-review prompts used for the judgments in this comparison project.')
  expect(pdf).toContain('1. Prompt 1')
  expect(pdf).toContain("Valid options: 'yes' | 'no' | 'maybe'")
  expect(pdf).not.toContain("Type: 'yes' | 'no' | 'maybe'")
  expect(pdf).not.toContain('Criteria: Population')
  expect(pdf).not.toContain('Disposition: combined')
  expect(pdf).toContain('Prompt:')
  expect(pdf).not.toContain('Original Text')
  ...
  expect(pdf.indexOf('Terminology')).toBeLessThan(
    pdf.indexOf('For more information about forska.ai, visit https://github.com/fc-io/forska.ai.'),
  )
  expect(pdf.indexOf('Prompts in this review')).toBeGreaterThan(
    pdf.indexOf('For more information about forska.ai, visit https://github.com/fc-io/forska.ai.'),
  )
  ...
  expect(pdf).toContain('Conflict resolution')
  expect(pdf).toContain('Current resolution: ')
  expect(pdf).toContain('/F2 10 Tf')
  expect(pdf).toContain('(Prompt 2)')
  const currentResolutionY = Number(pdf.match(/BT \/F1 10 Tf 48\.00 (\d+\.\d+) Td \(Current resolution: \) Tj ET/)?.[1])
  const currentResolutionAnswerY = Number(pdf.match(/BT \/F2 10 Tf \d+\.\d+ (\d+\.\d+) Td \(Prompt 2\) Tj ET/)?.[1])
  const chooseResolutionY = Number(pdf.match(/BT \/F1 10 Tf 48\.00 (\d+\.\d+) Td \(Choose resolution\) Tj ET/)?.[1])
  expect(currentResolutionAnswerY).toBe(currentResolutionY)
  expect(currentResolutionY - chooseResolutionY).toBeGreaterThanOrEqual(18)
...
test('summary comparison project export streams synthetic summary csv columns', async () => {
  ...
  expect(csv.trim().split('\n')).toEqual([
    [
      ...
      'Abstract/Summary',
      'Date added',
      'Overall decision - LLM - Model 1 - Article Title and Abstract',
      'Overall decision - LLM - Model 2 - Article Title and Abstract',
      'Summary Source - Overall decision - Human answer - Article Title and Abstract',
    ].join(','),
