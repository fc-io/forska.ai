...
const getReviewsHealthWorkloadContext = (
  projectId: string,
  operation: 'enabledPromptCount' | 'hasCuratedArticles' | 'hasRouteArticles',
): DuckdbWorkloadContext => {
  return {
    fallbackIntent: 'reject',
    maxResultRows: 1,
    projectId,
    routeOrJobKey: `projects.reviewsHealth.${operation}`,
    workloadClass: 'owner.product.reviewHealth',
  }
}
...
const getEnabledPromptCount = async (projectId: string): Promise<number> => {
  const rows = await getAppDatabaseService().queryJson<{count: number}>(
    ...
    WHERE project_id = '${escapeSqlString(projectId)}'
      AND enabled = TRUE
  `,
    getReviewsHealthWorkloadContext(projectId, 'enabledPromptCount'),
  )
...
const getHasCuratedArticles = async (projectId: string): Promise<boolean> => {
  const rows = await getAppDatabaseService().queryJson<{articleId: string}>(
    ...
    LIMIT 1
  `,
    getReviewsHealthWorkloadContext(projectId, 'hasCuratedArticles'),
  )
...
const getHasRouteArticles = async (projectId: string): Promise<boolean> => {
  const rows = await getAppDatabaseService().queryJson<{articleId: string}>(
    ...
    LIMIT 1
  `,
    getReviewsHealthWorkloadContext(projectId, 'hasRouteArticles'),
  )
