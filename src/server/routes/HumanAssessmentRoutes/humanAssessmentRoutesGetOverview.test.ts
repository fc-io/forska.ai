...
test('human assessment overview reads V4 human count contracts instead of raw judgment aggregates', async () => {
  ...
  expect(joinedStatements).toContain('FROM mart.review_article_filter_posting_serving_v4 human_status_posting')
  expect(joinedStatements).toContain("human_status_posting.filter_kind = 'humanStatus'")
  expect(joinedStatements).toContain("human_status_posting.filter_value = 'answered'")
  expect(joinedStatements).not.toContain('FROM mart.review_article_filter_posting_serving_v4 llm_status_posting')
...
test('human assessment both-project overview reads V4 both count contracts', async () => {
  ...
  expect(statements.join('\n')).toContain('FROM mart.review_article_filter_posting_serving_v4 human_status_posting')
  expect(statements.join('\n')).toContain('FROM mart.review_article_filter_posting_serving_v4 llm_status_posting')
  expect(statements.join('\n')).toContain("human_status_posting.filter_kind = 'humanStatus'")
  expect(statements.join('\n')).toContain("llm_status_posting.filter_kind = 'llmStatus'")
