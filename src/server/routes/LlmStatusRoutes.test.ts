...
test('llm status returns a bounded fallback while a slow foreground diagnostic refresh continues', async () => {
  ...
  try {
    ...
    expect(result.cachedBody.hasMetricsCompatibleRuntime).toBe(true)
...
test('llm status fresh query bypasses the cached latest-50 response', async () => {
  const tempDirectory = mkdtempSync(join(tmpdir(), 'llm-status-routes-'))
  const outputPath = join(tempDirectory, 'result.json')
  ...
  const runScript = globalThis.Bun.spawnSync(
    [
      ...
      `
                  ...
                  return [
                    {
                      ...
                      prefillTps: null,
                      rps: null,
                      ts: rowReadCount === 1 ? '2026-09-05T11:00:00.000Z' : '2026-09-05T11:00:30.000Z',
                      utilization: null,
                    },
                  ]
                },
        ...
        const {__resetLlmStatusCacheForTests, llmStatusRoutes} = await import(
          './src/server/routes/LlmStatusRoutes.ts?fresh-cache=' + Date.now()
        )
        __resetLlmStatusCacheForTests()

        const cachedResponse = await llmStatusRoutes.handle(new Request('http://localhost/api/llmstatus'))
        const cachedBody = await cachedResponse.json()
        const freshResponse = await llmStatusRoutes.handle(new Request('http://localhost/api/llmstatus?fresh=1'))
  ...
  try {
    ...
    const result = JSON.parse(readFileSync(outputPath, 'utf8')) as {
      cachedBody: {data: Array<{ts: string}>}
      freshBody: {data: Array<{ts: string}>}
      rowReadCount: number
    }

    expect(result.cachedBody.data[0]?.ts).toBe('2026-09-05T11:00:00.000Z')
    expect(result.freshBody.data[0]?.ts).toBe('2026-09-05T11:00:30.000Z')
    expect(result.rowReadCount).toBe(2)
  } finally {
    rmSync(tempDirectory, {force: true, recursive: true})
  }
})
...
test('llm status reports missing ingested rows for a metrics-compatible running job', async () => {
  ...
  try {
    ...
    expect(body.hasMetricsCompatibleRuntime).toBe(true)
...
test('llm status reports stale rows against an enabled runtime source without a running job', async () => {
  const tempDirectory = mkdtempSync(join(tmpdir(), 'llm-status-routes-'))
  const outputPath = join(tempDirectory, 'result.json')
  ...
  const runScript = globalThis.Bun.spawnSync(
    [
      ...
      `
        ...
        void mock.module(appDatabaseServiceModulePath, () => {
          return {
            getAppDatabaseService: () => {
              return {
                queryJson: async (statement) => {
                  if (statement.includes('COUNT(*) AS count') && statement.includes('FROM app.judgment_job')) {
                    return [{count: 0}]
                  }

                  if (statement.includes('COUNT(*) AS count') && statement.includes('FROM app.provider_connection pc')) {
                    return [{count: 1}]
                  }
        ...
        const {__resetLlmStatusCacheForTests, llmStatusRoutes} = await import(
          './src/server/routes/LlmStatusRoutes.ts?runtime-source=' + Date.now()
        )
        __resetLlmStatusCacheForTests()
  ...
  try {
    ...
    const body = JSON.parse(readFileSync(outputPath, 'utf8')) as {
      hasMetricsCompatibleJob: boolean
      hasMetricsCompatibleRuntime: boolean
      metadata: {isStale: boolean; staleMessage: string | null; staleReason: string | null; tableExists: boolean | null}
    }

    expect(body.hasMetricsCompatibleJob).toBe(false)
    expect(body.hasMetricsCompatibleRuntime).toBe(true)
    expect(body.metadata).toMatchObject({isStale: true, staleReason: 'latest-row-stale', tableExists: true})
    expect(body.metadata.staleMessage).toContain('enabled SGLang runtime source exists')
    expect(body.metadata.staleMessage).toContain('sample /metrics')
  } finally {
    rmSync(tempDirectory, {force: true, recursive: true})
  }
})
