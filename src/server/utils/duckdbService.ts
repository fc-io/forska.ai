...
const duckdbStartupIndexedTableRepairSpecs: DuckdbStartupIndexedTableRepairSpec[] = [
  ...
  },
  {
    ...
    `,
    mutationProbeSql: getSelectedImportCurrentStartupMutationProbeSql(),
    recreateRepairPrimaryKeyIndex: false,
    recreateSecondaryIndexes: false,
    schemaName: 'mart',
    ...
    const getPreservedCriticalProtectedTableRowCounts = async (repairedTableKeys) => {
      ...
      try {
        preservedInstance = await createInitializedDuckdbInstance({
          expectedEngine: ${JSON.stringify(duckdbExpectedEngineIdentity)},
...
const getDuckdbStartupFileLockProbeScript = () => {
  return `
    ...
    const instance = await createInitializedDuckdbInstance({create: DuckDBInstance.create.bind(DuckDBInstance), databasePath, options, expectedEngine: ${JSON.stringify(duckdbExpectedEngineIdentity)}})
    instance.closeSync()
  `
}
