...
const duckdbStartupIndexedTableRepairSpecs: DuckdbStartupIndexedTableRepairSpec[] = [
  ...
  },
  {
    ...
    mutationProbeSql: `
      ...
      FROM fallback_row;
      BEGIN;
      UPDATE mart.review_selected_article_import_current_v4
      SET selected_import_updated_at = current_timestamp
      WHERE EXISTS (
        SELECT 1
        FROM startup_probe_review_selected_article_import_current_v4 probe
        WHERE mart.review_selected_article_import_current_v4.project_id IS NOT DISTINCT FROM probe.project_id
          AND mart.review_selected_article_import_current_v4.project_scope_identity IS NOT DISTINCT FROM probe.project_scope_identity
          AND mart.review_selected_article_import_current_v4.selected_import_snapshot_id IS NOT DISTINCT FROM probe.selected_import_snapshot_id
          AND mart.review_selected_article_import_current_v4.article_id IS NOT DISTINCT FROM probe.article_id
      );
...
const markDuckdbStartupRepairForFatalIndexedTableError = (error: unknown) => {
  ...
  const repairSpecs = isIndexDeleteFailure
    ? getDuckdbStartupRepairSpecsForFatalIndexedTableError(
        normalizedError,
  ...
  const [primaryRepairSpec] = repairSpecs

  if (primaryRepairSpec === undefined) {
    return
  }
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      schemaName: primaryRepairSpec.schemaName,
      tableName: primaryRepairSpec.tableName,
    }),
  )
  writeRuntimeOperatorLogEvent({
    attrs: {
      ...
      markerPath,
      repairedTable: `${primaryRepairSpec.schemaName}.${primaryRepairSpec.tableName}`,
  ...
  const uniqueRepairSpecs = repairSpecs.filter((repairSpec, index) => {
    return (
      repairSpecs.findIndex((candidate) => {
        return candidate.schemaName === repairSpec.schemaName && candidate.tableName === repairSpec.tableName
      }) === index
    )
  })
...
const getDuckdbStartupPreflightScript = () => {
  return `
    ...
    const getObsoleteRepairPrimaryKeyIndexNames = async (spec) => {
      ...
      return rows
        .map((row) => {
          return typeof row.indexName === 'string' ? row.indexName : null
        })
        .filter((indexName) => {
          return indexName !== null && indexName.startsWith(repairedPrimaryKeyIndexPrefix)
        })
    }

    const dropObsoleteRepairPrimaryKeyIndexes = async (spec) => {
      const indexNames = await getObsoleteRepairPrimaryKeyIndexNames(spec)

      for (const indexName of indexNames) {
        await connection.run('DROP INDEX IF EXISTS ' + quoteIdentifier(spec.schemaName) + '.' + quoteIdentifier(indexName))
      }
    }
    ...
    const needsInlinePrimaryKeyRepairBeforeMutation = async (spec) => {
      ...
      return (
        ...
        || (spec.recreateRepairPrimaryKeyIndex === true && !(await hasUniqueIndexForColumns(spec)))
      )
    }
      ...
      for (const spec of tableRepairSpecs) {
        if (await tableExists(spec.schemaName, spec.tableName)) {
          if (await schemaRequirementsSatisfied(spec.schemaRequirements)) {
            ...
            await dropObsoleteRepairPrimaryKeyIndexes(spec)
  ...
  return `
    ...
    const assertRepairPostconditions = async (spec) => {
      ...
      if (primaryKeyColumns.length > 0) {
        ...
        if (spec.recreateRepairPrimaryKeyIndex === true && !(await hasUniqueIndexForPrimaryKeyColumns(spec))) {
...
const recordDuckdbControlTransactionStatement = (duckdbConnection: DuckDBConnection, statement: string) => {
  ...
  if (/^BEGIN\b/i.test(normalizedStatement)) {
    ...
    if (duckdbServiceState.controlTransactionDepth === 1) {
      duckdbServiceState.controlTransactionIndexedMutationTarget = null
      duckdbServiceState.controlTransactionIndexedMutationTargets = []
    }
    return
  ...
  if (/^(COMMIT|ROLLBACK)\b/i.test(normalizedStatement)) {
    ...
    if (duckdbServiceState.controlTransactionDepth === 0) {
      duckdbServiceState.controlTransactionIndexedMutationTarget = null
      duckdbServiceState.controlTransactionIndexedMutationTargets = []
    }
  }
