...
test('duckdb service uses one thread for the 6400MiB worker profile', () => {
  ...
  try {
    ...
    const runtimeConfig = JSON.parse(stdout) as {serializeConcurrentWork: boolean; threads: string}
