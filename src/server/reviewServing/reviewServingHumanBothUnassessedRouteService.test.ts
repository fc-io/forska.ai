...
test('human, both, and unassessed routes read one cursor page for numeric direct page jumps', async () => {
  ...
  const humanSql = humanReader.statements.join('\n')
  const bothSql = bothReader.statements.join('\n')
  const unassessedSql = unassessedReader.statements.join('\n')

  expect(humanSql).not.toContain('OFFSET')
  expect(bothSql).not.toContain('OFFSET')
  expect(unassessedSql).not.toContain('OFFSET')
  expect(humanSql).toContain('SELECT COUNT(DISTINCT serving.article_id) AS totalCount')
  expect(bothSql).toContain('SELECT COUNT(DISTINCT serving.article_id) AS totalCount')
  expect(humanSql).not.toContain('FROM mart.review_filtered_count_serving_v4')
  expect(bothSql).not.toContain('FROM mart.review_filtered_count_serving_v4')
})
