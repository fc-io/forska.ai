...
test('display routine updates write component-narrow patches for only claimed articles', async () => {
  ...
  const updateStatement = statements.find((statement) => {
    return statement.includes('UPDATE mart.review_article_serving_base_v4')
  })
...
test('display base rows flow through writer with display fields and selected import hot projection', async () => {
  ...
  const deletes = statements.filter((statement) => {
    return statement.includes('DELETE FROM mart.review_article_serving_')
  })
  ...
  const baseInsert = inserts.find((statement) => {
  ...
  expect(baseInsert).not.toContain('llm_status_key = excluded.llm_status_key')
  expect(baseInsert).not.toContain('human_status_key = excluded.human_status_key')
  expect(baseInsert).not.toContain('review_opened')
  expect(baseInsert).not.toContain('review_sections_completed')
  expect(baseInsert).not.toContain('mart.review_selected_import_patch_v4')
  expect(baseInsert).not.toContain('selected_patch')
