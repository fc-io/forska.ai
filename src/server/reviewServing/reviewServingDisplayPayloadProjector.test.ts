...
test('display routine updates write component-narrow patches for only claimed articles', async () => {
  ...
  expect(selectStatement).toContain('ELSE COALESCE(selected_hot.article_title, selected_base.article_title)')
  expect(selectStatement).toContain('ELSE COALESCE(selected_hot.external_id, selected_base.external_id)')
  expect(selectStatement).toContain('ELSE COALESCE(selected_hot.journal_title, selected_base.journal_title)')
  expect(selectStatement).toContain('ELSE COALESCE(selected_hot.publication_year, selected_base.publication_year)')
  expect(selectStatement).toContain(
...
test('display base rows flow through writer with display fields and selected import hot projection', async () => {
  ...
  expect(selectStatement).toContain('LEFT JOIN app.review_import_article_hot_field selected_hot')
  expect(selectStatement).toContain('COALESCE(selected_hot.article_title, selected_base.article_title)')
  expect(selectStatement).toContain('COALESCE(selected_hot.external_id, selected_base.external_id)')
  expect(selectStatement).toContain('COALESCE(selected_hot.journal_title, selected_base.journal_title)')
  expect(selectStatement).toContain('COALESCE(selected_hot.publication_year, selected_base.publication_year)')
