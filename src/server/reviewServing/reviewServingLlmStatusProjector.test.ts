...
test('prompt config claims rebuild only prompt-scoped LLM status rows', async () => {
  ...
  expect(promptSelect).toContain('FROM app."judgment"')
  expect(promptSelect).toContain('judgment.prompt_id = dirty_prompt.prompt_id')
  expect(promptSelect).toContain('ROW_NUMBER() OVER')
  expect(promptSelect).toContain('ORDER BY created_at DESC NULLS LAST, id DESC')
  expect(promptSelect).toContain('judgment.judgment_rank = 1')
