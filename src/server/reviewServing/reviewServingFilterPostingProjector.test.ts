...
test('full posting rebuilds write serving without contribution or incremental patch fanout', async () => {
  ...
  expect(joined).not.toContain('article_judgment_status AS')
  expect(joined).not.toContain('enabled_prompt_count AS')
  expect(joined).toContain('list_mode_state.llm_status')
  expect(joined).toContain('list_mode_state.human_status')
  expect(joined).toContain('list_mode_state.llm_has_judgment')
  expect(joined).toContain('serving.llm_status AS filterValue')
  expect(joined).toContain('serving.human_status AS filterValue')
  expect(joined).toContain('CAST(serving.llm_has_judgment AS VARCHAR) AS filterValue')
...
test('status postings reuse list-mode state instead of recomputing from optional payload details', async () => {
  const {database, statements} = createPostingDatabase({newRows: []})
  ...
  expect(selectStatement).toContain('list_mode_state.llm_status')
  expect(selectStatement).toContain('list_mode_state.human_status')
  expect(selectStatement).toContain('list_mode_state.llm_has_judgment')
  expect(selectStatement).toContain('judgment_detail_source AS')
  expect(selectStatement).not.toContain('article_judgment_status AS')
  expect(selectStatement).not.toContain('llm_answered_prompt_count')
  expect(selectStatement).not.toContain('human_answered_prompt_count')
  ...
  expect(selectStatement).toContain('serving.llm_status AS filterValue')
  expect(selectStatement).toContain("'llmHasJudgment' AS filterKind")
  expect(selectStatement).toContain('CAST(serving.llm_has_judgment AS VARCHAR) AS filterValue')
  expect(selectStatement).toContain("'humanStatus' AS filterKind")
  expect(selectStatement).toContain('serving.human_status AS filterValue')
  expect(selectStatement).not.toContain('enabled_prompt_count AS')
