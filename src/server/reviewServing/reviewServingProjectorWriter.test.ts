...
test('title search rebuild ranges insert rows with conflict-ignore and no scoped indexed deletes', async () => {
  ...
  expect(workloadContexts).toEqual([])
  expect(
...
test('projector writer updates rows, manifests, acknowledgements, watermarks, and promotion in one transaction', async () => {
  ...
  expect(
    statements.some((statement) => {
      return (
        statement.includes('INSERT INTO app.review_serving_projector_watermark')
...
test('projector writer uses larger scan-guarded insert-missing batches for summary contribution partials', async () => {
  ...
  const result = await writeReviewServingProjectorComponent({component: 'summary', records, statements: []}, database)
