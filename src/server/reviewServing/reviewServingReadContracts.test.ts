...
test('title-search row contracts keep ordering and cursors on primary article serving rows', () => {
  const titleSearchRowContracts = reviewServingReadContractList.filter((contract) => {
    return (
      ...
      && contract.allowedFilters.includes('searchTokenPrefix')
...
test('human payload contracts cover list and detail response judgments', () => {
  ...
  expect(llmListJudgments?.requiredComponents).toEqual(['llmStatus', 'payload'])
  expect(humanListJudgments?.requiredComponents).toEqual(['humanStatus', 'payload'])
  expect(bothListJudgments?.requiredComponents).toEqual(['llmStatus', 'humanStatus', 'payload'])
  expect(bothListHumanJudgments?.requiredComponents).toEqual(['llmStatus', 'humanStatus', 'payload'])
  expect(detailHumanJudgments?.requiredComponents).toEqual(['humanStatus', 'summary', 'payload'])
  expect(llmListJudgments?.requiredComponents).not.toContain('summary')
  expect(humanListJudgments?.requiredComponents).not.toContain('summary')
  expect(bothListJudgments?.requiredComponents).not.toContain('summary')
  expect(bothListHumanJudgments?.requiredComponents).not.toContain('summary')
