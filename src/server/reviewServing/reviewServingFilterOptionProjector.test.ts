...
test('source query preserves active search and filter scope without using posting rows as response rows', async () => {
  ...
  expect(sourceStatement).toContain("'importRoute' AS facetKey, selected_base.import_route_id AS facetValue")
  expect(sourceStatement).toContain('selected_base.import_route_id IS NOT NULL')
