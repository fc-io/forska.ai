...
test('no-search option projection reuses finalized facet rows for migrated facet option identities', async () => {
  ...
  expect(fallbackStatement).toContain('FROM mart.review_article_filter_posting_serving_v4 posting')
  expect(fallbackStatement).toContain("posting.filter_kind = 'llmStatus'")
  expect(fallbackStatement).toContain("posting.filter_kind = 'humanStatus'")
  expect(fallbackStatement).toContain("concat('review:llmStatus:', posting.filter_value)")
  expect(fallbackStatement).toContain("concat('review', ':humanStatus:', posting.filter_value)")
  expect(fallbackStatement).toContain('COUNT(DISTINCT posting.article_id) AS countValue')
...
test('source query preserves active search and filter scope without using posting rows as response rows', async () => {
  ...
  expect(sourceStatement).toContain('FROM mart.review_article_filter_posting_serving_v4 posting')
  expect(sourceStatement).toContain("posting.filter_kind = 'llmStatus'")
  expect(sourceStatement).toContain("posting.filter_kind = 'humanStatus'")
  expect(sourceStatement).toContain('posting.filter_value AS facetValue')
  expect(sourceStatement).toContain('COUNT(DISTINCT posting.article_id) AS countValue')
