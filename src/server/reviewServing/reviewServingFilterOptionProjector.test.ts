...
test('no-search option projection reuses finalized facet rows for migrated facet option identities', async () => {
  ...
  expect(fallbackStatement).toContain('AND (list_mode_state.has_llm_list_mode IS TRUE)')
  expect(fallbackStatement).not.toContain('INNER JOIN LATERAL')
...
test('source query preserves active search and filter scope without using posting rows as response rows', async () => {
  ...
  expect(sourceStatement).toContain(
    'AND (list_mode_state.has_llm_list_mode IS TRUE OR list_mode_state.has_human_list_mode IS TRUE)',
  )
  expect(sourceStatement).not.toContain('INNER JOIN LATERAL')
