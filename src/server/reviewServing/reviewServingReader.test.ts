...
test('readReviewServingRows admits ready manifests and executes serving SQL only after shape assertion', async () => {
  ...
  expect(reader.statements[0]).toContain('selected_import.project_id = mart.review_article_serving_v4.project_id')
  ...
  expect(reader.statements[0]).toContain('selected_import.article_id = mart.review_article_serving_v4.article_id')
...
test('readReviewServingRows applies ordered-prefix filters and mixed-direction cursor predicates', async () => {
  ...
  expect(sql).toContain('filter_state_0.duplicate_flag IS TRUE')
  expect(sql).toContain("filter_state_2.llm_status IN (SELECT unnest(['answered']::VARCHAR[]))")
  expect(sql).toContain("filter_4.filter_kind = 'importRoute'")
  expect(sql).toContain("filter_4.filter_value IN (SELECT unnest(['import-route-1']::VARCHAR[]))")
  expect(sql).toContain('list_contains(filter_4.article_ids, mart.review_article_serving_v4.article_id)')
