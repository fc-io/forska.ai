...
test('readReviewServingRows applies ordered-prefix filters and mixed-direction cursor predicates', async () => {
  ...
  expect(sql).toContain("filter_2.filter_kind = 'llmStatus'")
  expect(sql).toContain("filter_2.filter_value IN (SELECT unnest(['answered']::VARCHAR[]))")
  ...
  expect(sql.match(/FROM mart\.review_article_filter_posting_serving_v4 filter_/gu)?.length).toBe(5)
