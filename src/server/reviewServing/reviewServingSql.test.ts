...
test('assertReviewServingSqlShape accepts serving-table keyset SQL', () => {
  ...
  expect(sql).toContain('payload.journal_title AS journal_title')
...
test('buildReviewServingRowsSql uses article ordering and payload hydration for prompt preview', () => {
  ...
  expect(sql).toContain('payload.arxiv_id AS arxiv_id')
  expect(sql).toContain('payload.biorxiv_id AS biorxiv_id')
  expect(sql).toContain('payload.medrxiv_id AS medrxiv_id')
  expect(sql).toContain('payload.doi AS doi')
  expect(sql).toContain('payload.pmid AS pmid')
  expect(sql).toContain('payload.url AS url')
  expect(sql).toContain('article.full_text_pdf AS full_text_pdf')
...
test('buildReviewServingRowsSql pins fixed list-mode judgment payload reads', () => {
  ...
  expect(bothListSql).toContain("AND mart.review_article_judgment_detail_serving_v4.list_mode_key = 'both'")
