...
test('judgment payload projection writes llm and human payload kinds with SQL-native statements', async () => {
  ...
  expect(joined).toContain('judgment_model_id')
  expect(joined).toContain('payload.model_id AS judgment_model_id')
...
test('judgment payload projection replaces only dirty article detail rows', async () => {
  ...
  expect(llmHydrationInsert).not.toContain('prompt_original_text')
  expect(llmHydrationInsert).not.toContain('prompt_heading')
  expect(llmHydrationInsert).not.toContain('prompt_type')
  expect(llmHydrationInsert).not.toContain('prompt_criteria_disposition')
  expect(llmHydrationInsert).not.toContain('JOIN app.project_prompt')
  expect(llmHydrationInsert).not.toContain('JOIN app.prompt')
...
test('human payload projection filters rows by active human judgment mode', async () => {
  ...
  expect(humanHydrationSelect).not.toContain('prompt_original_text')
  expect(humanHydrationSelect).not.toContain('prompt_heading')
  expect(humanHydrationSelect).not.toContain('prompt_type')
  expect(humanHydrationSelect).not.toContain('prompt_criteria_disposition')
  expect(humanHydrationSelect).not.toContain('JOIN app.project_prompt')
  expect(humanHydrationSelect).not.toContain('JOIN app.prompt')
