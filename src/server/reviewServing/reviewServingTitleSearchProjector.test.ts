...
test('title search projection writes token rows and search-only component state for dirty articles', async () => {
  ...
  expect(selectStatement).toContain('ELSE COALESCE(selected_hot.article_title, selected_base.article_title, article.article_title)')
...
test('sql-native title search rebuild clears stale chunk tokens before inserting current tokens', async () => {
  ...
  expect(insertStatement).toContain('COALESCE(selected_hot.article_title, selected_base.article_title, article.article_title)')
