...
type ReviewServingProjectorWorkerDependencies = {
  ...
  waitForForegroundQueue?: typeof waitForDuckdbForegroundQueue
...
const getAliasedSnapshotIdPredicate = (alias: string, snapshotIds: readonly string[]) => {
  return snapshotIds.length === 0
    ? 'FALSE'
    : `${alias}.snapshot_id IN (${snapshotIds
        .map((snapshotId) => {
          return getSqlLiteral(snapshotId)
        })
        .join(', ')})`
  ...
  const [row] = await database.queryJson<RebuildChunkOutputChecksumRow>(`
    SELECT
      ...
      sha256(COALESCE(string_agg(
        ...
        COALESCE(CAST(detail.judgment_id AS VARCHAR), '') || ':' ||
...
const runSearchRebuildChunk = async (
  ...
  const projectId = requireRebuildChunkProjectId(input.chunk)
  await requireRebuildChunkProjectionManifest(input.chunk, database)
  const snapshots = await getRebuildChunkSnapshots(input.chunk, database)
  const snapshotIds = snapshots.map((snapshot) => {
    return snapshot.snapshotId
  })
  ...
  const completedChunk = await writeReviewServingRebuildChunkOutput(
    {
      ...
      validateOutput: async (tx) => {
        return getRebuildChunkOutputValidation({
          chunk: input.chunk,
          getChecksum: () => {
            return getSummaryRebuildChunkOutputChecksum({chunk: input.chunk, snapshotIds: input.snapshotIds}, tx)
          },
          getCount: () => {
            return getSummaryRebuildChunkOutputCount({chunk: input.chunk, snapshotIds: input.snapshotIds}, tx)
          },
        })
      },
      writeOutput: async () => {
        return {diagnosticsJson: {summaryBatchWriter: {writeOutputAlreadyCompleted: true}}}
      },
    },
    database,
  )
...
const runSummaryRebuildChunkBatch = async (
  input: {chunks: readonly ReviewServingRebuildChunkManifest[]; leaseOwner: string},
  database: ReviewServingChunkManifestRepositoryDatabase & ReviewServingProjectorWorkerDatabase,
) => {
  if (!canRunSummaryRebuildChunkBatch(input.chunks)) {
    return null
  }
...
const getLoggedReviewServingProjectorRunners = (database: ReviewServingProjectorWorkerDatabase) => {
  const runners = getDefaultReviewServingProjectorRunners(database)

  for (const component of Object.keys(runners) as ReviewServingProjectionComponent[]) {
    const runner = runners[component]

    if (runner !== undefined) {
      runners[component] = (context) => {
        return runReviewServingProjectorWorkerCyclePhase(`projector:${component}`, () => {
...
const runReviewServingProjectorWorkerCyclePhase = async <T>(phase: string, operation: () => Promise<T>) => {
  reviewServingProjectorWorkerTimingLogger.log(
    `review-serving-projector-worker:cycle-phase:${phase}:started`,
    '[reviewServingProjectorWorker] cycle phase started',
    {component: 'reviewServingProjectorWorker', event: 'cyclePhaseStarted', phase},
  )
  ...
  try {
    ...
    reviewServingProjectorWorkerTimingLogger.log(
      `review-serving-projector-worker:cycle-phase:${phase}:failed`,
      '[reviewServingProjectorWorker] cycle phase failed',
      {
        component: 'reviewServingProjectorWorker',
        durationMs: Math.max(0, Date.now() - startedAtMs),
        error,
        event: 'cyclePhaseFailed',
        phase,
      },
    )
    throw error
  ...
  reviewServingProjectorWorkerTimingLogger.log(
    `review-serving-projector-worker:cycle-phase:${phase}:completed`,
    '[reviewServingProjectorWorker] cycle phase completed',
    {
      component: 'reviewServingProjectorWorker',
      durationMs: Math.max(0, Date.now() - startedAtMs),
      event: 'cyclePhaseCompleted',
      phase,
    },
  )
...
const combineReviewServingProjectorWakeResults = (
  first: WakeReviewServingProjectorServiceResult | null,
  ...
  const releasedCount = first.releasedClaimIds.length + second.releasedClaimIds.length
  const runCount = first.runs.length + second.runs.length
  const status =
    failureCount > 0
      ? 'failed'
      : releasedCount > 0 || first.status === 'partial' || second.status === 'partial'
        ? 'partial'
...
const getReviewServingProjectorWorkerAdmissionSnapshot = (
  ...
  return {
    ...
    ranBeforeChunks,
    starvationAdmitted: admission.starvationAdmitted || ranBeforeChunks,
  }
}
...
const logReviewServingProjectorWorkerRebuildChunkStarted = (input: {
  chunk: ReviewServingRebuildChunkManifest
  workerId: string
}) => {
  reviewServingProjectorWorkerCycleLogger.log(
    `review-serving-projector-worker:rebuild-chunk-started:${input.chunk.requestId ?? 'no-request'}:${input.chunk.projectionComponent}`,
    '[reviewServingProjectorWorker] rebuild chunk started',
    {
      chunkEndKey: input.chunk.chunkEndKey,
      chunkId: input.chunk.chunkId,
      chunkStartKey: input.chunk.chunkStartKey,
      component: input.chunk.projectionComponent,
      estimatedInputRows: input.chunk.estimatedInputRows,
      estimatedOutputRows: input.chunk.estimatedOutputRows,
      event: 'rebuildChunkStarted',
      requestId: input.chunk.requestId,
      splitDepth: input.chunk.splitDepth,
      workerId: input.workerId,
    },
...
const isCompatibleReviewServingProjectorWorkerRebuildChunkBatchInput = (
  ...
  const isRequestlessSummaryBatch =
    firstChunk !== undefined
    && firstChunk.projectionComponent === 'summary'
    && (firstChunk.requestId === null || (nextChunk.requestId ?? null) === null)

  return (
    firstChunk !== undefined
    && isCompatibleReviewServingProjectorWorkerRebuildRequestBatch(firstChunk, nextChunk)
    && !isRequestlessSummaryBatch
...
const claimReviewServingProjectorWorkerRebuildChunkInput = async ({
  ...
  const completed = await runReviewServingProjectorWorkerCyclePhase('checkRebuildChunkCompletion', () => {
    return measureReviewServingProjectorWorkerPhase(timings, 'claimCompletionCheckMs', async () => {
      return service.isChunkComplete(chunkInput, database)
    })
  })
  const claimedChunk = completed
    ? null
    : await runReviewServingProjectorWorkerCyclePhase('claimRebuildChunkLease', () => {
        return measureReviewServingProjectorWorkerPhase(timings, 'claimUpdateMs', async () => {
          return service.claimChunk(
            {
              ...chunkInput,
              leaseExpiresAt: getLeaseExpiresAt(options),
              leaseOwner: workerId,
...
const runClaimedReviewServingProjectorWorkerRebuildChunk = async ({
  ...
  try {
    ...
    logReviewServingProjectorWorkerRebuildChunkStarted({chunk: effectiveClaimedChunk, workerId})
...
const runSummaryReviewServingProjectorWorkerRebuildChunkBatch = async (input: {
  claimedChunks: readonly ClaimedReviewServingProjectorWorkerRebuildChunk[]
  database: ReviewServingChunkManifestRepositoryDatabase & ReviewServingProjectorWorkerDatabase
  dependencies: ReviewServingProjectorWorkerDependencies
  options: ReviewServingProjectorWorkerCycleOptions
...
const getWorkerProjectorServiceDependencies = (input: {
  ...
  return {
    ...
    database: input.database,
    nowMs: () => {
      return getWorkerNowMs(input.dependencies, input.options)
    },
  }
}
...
export const runReviewServingProjectorWorkerCycle = async (
  ...
  await runReviewServingProjectorWorkerCyclePhase('repairRequestlessBootstrapAdoptions', () => {
    return repairRequestlessBootstrapRebuildAdoptions({database, projectId: options.rebuildProjectId})
  })
  const terminalFailedChunk = await runReviewServingProjectorWorkerCyclePhase('finalizeTerminalFailedRequests', () => {
    return finalizeTerminalFailedRebuildRequests({database, projectId: options.rebuildProjectId})
  })
  if (terminalFailedChunk === null) {
    await runReviewServingProjectorWorkerCyclePhase('readmitRetryableFailedRequests', () => {
  ...
  const projectorServiceDependencies = getWorkerProjectorServiceDependencies({
    admissionDeadlineMs,
    database,
    dependencies,
    options,
