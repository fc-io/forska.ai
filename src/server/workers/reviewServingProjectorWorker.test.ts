...
const isJobDrivenDirtyWorkPendingStatement = (statement: string) => {
  return (
    statement.includes('FROM app.review_serving_dirty_work_claim_state state')
    && statement.includes('INNER JOIN app.judgment_job job')
  )
}

const fakeStaleCandidateCleanupResult = {
  failedSnapshots: [{projectId: 'project-1', referenceSnapshotId: 'snapshot-active', snapshotId: 'snapshot-stale'}],
...
const createWorkerHarness = (input?: {
  ...
  const dependencies: ReviewServingProjectorWorkerDependencies = {
    ...
    cleanupStaleCandidateSnapshots: async (cleanupInput) => {
      ...
      if (input?.staleCandidateCleanupThrows) {
        throw new Error('stale candidate cleanup failed')
      }
...
test('worker runs fresh job-driven llmStatus dirty work before rebuild backlog chunks without starving chunks', async () => {
  ...
  harness.database.queryJson = async <T>(statement: string, workloadContext?: DuckdbWorkloadContext) => {
    ...
    if (
      ...
      return [{projectionComponent: 'llmStatus'}] as T[]
    }
...
const runJobDrivenDirtyWorkCycle = async (input: {
  ...
  harness.dependencies.projectorServiceDependencies = {
    ...
    runners: {
      ...
      queue: async () => {
        projectedComponents.push('queue')
        nowMs += 300
  ...
  harness.dependencies.wakeProjectors = async (wakeInput, serviceDependencies) => {
    ...
    return wakeReviewServingProjectorService(wakeInput, serviceDependencies)
  }
...
test('job-driven llmStatus and queue dirty work both progress in one cycle before the rebuild chunk uses up the wake budget', async () => {
  const cycle = await runJobDrivenDirtyWorkCycle({componentRotationOffset: 0, llmStatusBatchMs: 2_000})

  expect(cycle.projectedComponents).toEqual(['llmStatus', 'queue'])
  expect(cycle.harness.runChunkInputs).toHaveLength(1)
  expect(cycle.harness.wakeInputs[0]).toMatchObject({componentOrder: ['llmStatus', 'queue'], maxWakeMs: 5_000})
...
test('job-driven dirty work alternates which component goes first so an llmStatus batch that exhausts the budget cannot starve queue', async () => {
  const llmStatusFirst = await runJobDrivenDirtyWorkCycle({componentRotationOffset: 0, llmStatusBatchMs: 5_000})
  const queueFirst = await runJobDrivenDirtyWorkCycle({componentRotationOffset: 1, llmStatusBatchMs: 5_000})
...
test('worker admits a starved projector wake once the starvation window passes while only foreground DuckDB work is queued', async () => {
  ...
  expect(starving.result.admission).toEqual({
    appendQueueDepth: 0,
    foregroundQueueDepth: 2,
  ...
  expect(admitted.result.admission).toEqual({
    appendQueueDepth: 0,
    foregroundQueueDepth: 2,
...
test('worker cycle log attrs carry the projector block reason and the queue depths at decision time', async () => {
  ...
  expect(getReviewServingProjectorWorkerCycleLogAttrs(foregroundYielded)).toMatchObject({
    ...
    projectorRanBeforeChunks: false,
...
const getFakeWakeResultForBudget = (maxWakeMs: number) => {
  return maxWakeMs <= 0
    ? {
...
test('worker preempts the rebuild chunk batch with a full-budget wake once budget-blocked wakes have starved for the starvation window', async () => {
  const harness = createWorkerHarness()
  const controller = new AbortController()
  const loopStartedAtMs = 1_000_000
  const maxWakeMs = 1_500
  const events: Array<{atMs: number; kind: 'chunk' | 'wake'; maxWakeMs: number | null}> = []
  let clockMs = loopStartedAtMs
  ...
  harness.dependencies.rebuildChunkService = {
    ...harness.dependencies.rebuildChunkService,
    runClaimedChunk: async ({chunk}) => {
      events.push({atMs: clockMs, kind: 'chunk', maxWakeMs: null})
      harness.runChunkInputs.push(chunk)
      clockMs += maxWakeMs
  ...
  harness.dependencies.wakeProjectors = async (wakeInput) => {
    harness.wakeInputs.push(wakeInput)
    events.push({atMs: clockMs, kind: 'wake', maxWakeMs: wakeInput.maxWakeMs})
  ...
  harness.dependencies.sleep = async () => {
    ...
    if (clockMs - loopStartedAtMs >= 34_000) {
      controller.abort()
    }
  }
  ...
  const budgetBlockedWakeTimes = events
    .filter((event) => {
      return event.kind === 'wake' && event.maxWakeMs === 0
    })
    .map((event) => {
      return event.atMs
    })
  const fullBudgetWakeIndex = events.findIndex((event) => {
    return event.kind === 'wake' && event.maxWakeMs === maxWakeMs
  })

  expect(budgetBlockedWakeTimes).toEqual([
    ...Array.from({length: 15}, (_, cycleIndex) => {
      return loopStartedAtMs + cycleIndex * 2_000 + maxWakeMs
    }),
    loopStartedAtMs + 32_000 + maxWakeMs,
  ])
  expect(events.slice(fullBudgetWakeIndex, fullBudgetWakeIndex + 2)).toEqual([
    {atMs: loopStartedAtMs + 30_000, kind: 'wake', maxWakeMs},
    {atMs: loopStartedAtMs + 30_000, kind: 'chunk', maxWakeMs: null},
  ])
  expect(
    events.filter((event) => {
      return event.kind === 'wake' && event.maxWakeMs === maxWakeMs
...
const runStarvedChunkCycle = async (input: {foregroundQueueDepth: number; pendingLlmStatusWakeMs?: number}) => {
  ...
  harness.database.queryJson = async <T>(statement: string) => {
    return (
      input.pendingLlmStatusWakeMs !== undefined && isJobDrivenDirtyWorkPendingStatement(statement)
...
const runActivationDrainCycle = async (lastAdmittedWakeAtMs: number) => {
  ...
  const foregroundDisplayChunkInput = {...chunkInput, requestId: 'rebuild:foreground-display'}
  const foregroundDisplayChunk = {
    ...chunkManifest,
    ...foregroundDisplayChunkInput,
  } satisfies ReviewServingRebuildChunkManifest
  let nowMs = 31_000
  ...
  harness.dependencies.rebuildChunkService = {
    ...harness.dependencies.rebuildChunkService,
    claimChunk: async () => {
      return foregroundDisplayChunk
    },
    getNextChunk: async () => {
      return foregroundDisplayChunkInput
    },
  ...
  const result = await runReviewServingProjectorWorkerOnce(
    {
      foregroundRebuildDrainChunkBudget: 4,
      foregroundRebuildDrainCompletedCount: 1,
      foregroundRebuildDrainStartedAtMs: 30_000,
  ...
  return {deltaPartitionStatements, events, harness, result}
}

test('a starved cycle wakes projectors before the chunk batch with the full budget and keeps maintenance running during an activation drain', async () => {
  const starved = await runActivationDrainCycle(1_000)

  expect(starved.events).toEqual(['wake', 'chunk'])
  expect(starved.harness.wakeInputs).toHaveLength(1)
  expect(starved.harness.wakeInputs[0]).toMatchObject({componentRotationOffset: 0, maxWakeMs: 1_500})
  expect((starved.harness.wakeInputs[0] as {componentOrder?: unknown}).componentOrder).toBeUndefined()
  expect(starved.deltaPartitionStatements).toHaveLength(2)
  expect(starved.harness.dirtyWorkRetentionCleanupInputs).toEqual([{}])
  expect(starved.result.chunk).toMatchObject({requestId: 'rebuild:foreground-display', status: 'completed'})
  expect(starved.result.projector).toMatchObject({blockedReason: null, status: 'completed'})
  expect(starved.result.admission).toEqual({
    appendQueueDepth: 0,
    foregroundQueueDepth: 0,
    ranBeforeChunks: true,
    starvationAdmitted: true,
  })
  expect(starved.result.nextAdmittedWakeAtMs).toBe(31_000)
  expect(getReviewServingProjectorWorkerCycleLogAttrs(starved.result)).toMatchObject({
    projectorBlockedReason: null,
    projectorRanBeforeChunks: true,
    projectorStarvationAdmitted: true,
  })
})

test('a cycle inside the starvation window keeps the chunk batch first and a budget-blocked wake does not reset the starvation timer', async () => {
  const normal = await runActivationDrainCycle(1_001)

  expect(normal.events).toEqual(['chunk', 'wake'])
  expect(normal.harness.wakeInputs).toHaveLength(1)
  expect(normal.harness.wakeInputs[0]).toMatchObject({maxWakeMs: 0})
  expect((normal.harness.wakeInputs[0] as {componentOrder?: readonly string[]}).componentOrder).toContain('payload')
  expect(normal.deltaPartitionStatements).toEqual([])
  expect(normal.harness.dirtyWorkRetentionCleanupInputs).toEqual([])
  expect(normal.result.projector).toMatchObject({blockedReason: 'budget', status: 'blocked'})
  expect(normal.result.admission).toEqual({
    appendQueueDepth: 0,
    foregroundQueueDepth: 0,
    ranBeforeChunks: false,
    starvationAdmitted: false,
  })
  expect(normal.result.nextAdmittedWakeAtMs).toBe(1_001)
  expect(getReviewServingProjectorWorkerCycleLogAttrs(normal.result)).toMatchObject({
    projectorBlockedReason: 'budget',
    projectorRanBeforeChunks: false,
    projectorStarvationAdmitted: false,
  })
})
...
test('worker recycles DuckDB after request-associated summary chunks at the RSS cap', async () => {
  const harness = createWorkerHarness({wakeStatus: 'completed'})
...
test('judgment input content rebuild checksum joins authoritative app scalars for strict validation', () => {
  ...
  expect(checksumSource).not.toContain('mart.review_article_judgment_detail_hydration_serving_v4')
  expect(checksumSource).toContain('LEFT JOIN app."judgment" llm_judgment')
  expect(checksumSource).toContain('llm_judgment.id = detail.judgment_id')
  expect(checksumSource).toContain('llm_judgment.article_id = detail.article_id')
  expect(checksumSource).toContain('llm_judgment.prompt_id = detail.prompt_id')
  expect(checksumSource).toContain('llm_judgment.deleted_at IS NULL')
  expect(checksumSource).toContain('LEFT JOIN app.judgment_assessment assessment')
  expect(checksumSource).toContain('LEFT JOIN app.model model')
  expect(checksumSource).toContain('LEFT JOIN app.provider_connection provider_connection')
  expect(checksumSource).toContain('LEFT JOIN app."judgment_human" human_judgment')
  expect(checksumSource).toContain('LEFT JOIN app."judgment_human_summary" human_summary')
  expect(checksumSource).toContain(
  ...
  expect(checksumSource).toContain('COALESCE(CAST(assessment.id AS VARCHAR)')
...
test('display rebuild chunk executor writes bounded base rows and completes the chunk', async () => {
  ...
  expect(joined).toContain("json_extract_string(component_state.value, '$.component') = 'display'")
  expect(joined).toContain("json_extract_string(component_state.value, '$.projectionIdentity') = 'display:project-1'")
  expect(joined).not.toContain("json_extract_string(snapshot.composed_identity_json, '$.display.projectionIdentity')")
...
test('selected import bootstrap rebuild chunk writes article range and completed snapshot state without patch fanout', async () => {
  ...
  expect(joined).toContain('DELETE FROM app.review_projection_identity_manifest')
