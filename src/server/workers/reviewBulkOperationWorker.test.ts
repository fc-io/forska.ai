...
test('review bulk operation worker selects add-to-project batches from persisted filter criteria', async () => {
  ...
  expect(joined).toContain("humanStatusFilter.filter_kind = 'humanStatus'")
  expect(joined).toContain("humanStatusFilter.filter_value = 'answered'")
  expect(joined).toContain("llmStatusFilter.filter_kind = 'llmStatus'")
  expect(joined).toContain("llmStatusFilter.filter_value = 'answered'")
