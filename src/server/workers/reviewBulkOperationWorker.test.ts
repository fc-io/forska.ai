...
test('review bulk operation worker selects add-to-project batches from persisted filter criteria', async () => {
  ...
  expect(joined).toContain('FROM mart.review_article_filter_state_serving_v4 humanStatusFilter')
  expect(joined).toContain("humanStatusFilter.human_status = 'answered'")
  expect(joined).toContain('FROM mart.review_article_filter_state_serving_v4 llmStatusFilter')
  expect(joined).toContain("llmStatusFilter.llm_status = 'answered'")
  expect(joined).not.toContain('s.human_status_key')
  expect(joined).not.toContain('s.llm_status_key')
  expect(joined).toContain('filter_0.duplicate_flag IS TRUE')
  expect(joined).toContain('filter_1.conflict_flag IS TRUE')
...
test('review bulk operation worker advances export jobs through bounded keyset selection', async () => {
  ...
  expect(joined).toContain('FROM mart.review_article_serving_v4 s')
  expect(joined).toContain("s.list_mode_key = 'llm'")
  expect(joined).toContain('FROM mart.review_article_judgment_detail_serving_v4 llm_judgment_detail')
  expect(joined).toContain("llm_judgment_detail.list_mode_key = 'llm'")
  expect(joined).toContain("llm_judgment_detail.payload_kind = 'llm'")
  expect(joined).toContain('llm_judgment_detail.article_id = s.article_id')
  expect(joined).toContain('llm_judgment_detail.placeholder_kind IS NULL')
  expect(joined).toContain('llm_judgment_detail.is_answered IS TRUE')
  expect(joined).not.toContain('s.llm_judged_prompt_count > 0')
  expect(joined).toContain('ORDER BY s.article_id ASC')
  expect(joined).toContain('LIMIT 2')
  expect(joined).toContain('processed_count = processed_count + 2')
