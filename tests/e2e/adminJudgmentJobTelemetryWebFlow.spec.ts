...
test('admin job detail web flow explains provider telemetry bottleneck states', async ({page}) => {
  ...
  try {
    for (const telemetryScenario of telemetryScenarios) {
      ...
      await expect(providerCapacityTelemetry.getByText(telemetryScenario.label, {exact: true})).toBeVisible()
      await expect(providerCapacityTelemetry.getByText(telemetryScenario.description)).toBeVisible()
