...
- Context: `claimReviewServingDirtyWork`, the 60 s `cleanupReviewServingDirtyWorkRetention` worker cleanup, and foreground `search` rebuild chunk batching on a live DB holding about 180k pending high-water dirty rows in two large projects plus 224k pending search chunks.
- Cause: The claim window was an unordered 2048-row cut of the whole component backlog picked by global lane age, so one newer row of a small project waited behind 180k older rows. High-water coalescing was off (`coalesceDirtyWorkLimit` 0) because the previous one-candidate-per-iteration `EXISTS` self-join loop was too expensive per cycle. Search chunks were claimed 4 per cycle regardless of memory.
- Fix: Project selection is one aggregate statement (`MIN(updated_at)` of eligible rows and `MAX(updated_at)` of served rows per project; never-served projects first, then least recently served, oldest eligible row, project id) followed by an `ORDER BY ... LIMIT 2048` Top-N window over that one project only, so memory is a 2048-row heap instead of a sort of the backlog. The served aggregate is bounded to claim-state rows updated in the last 7 days, counts `released` rows as served and ignores `superseded_by_high_water` / `orphan_missing_component` cleanup completions; claim-state rows whose dirty-work row no longer claims are re-maintained or deleted in the same transaction so a phantom row cannot be reselected every wake. Coalescing is one bounded statement over `review_serving_dirty_work` rows with `scope_kind = 'project' AND article_id IS NULL` only (219 of the 1,250,202 pending rows in the live snapshot; the 1,249,695 article-scoped rows are claimed per article and must never be superseded): a per-lane `MAX(struct_pack(high_water, updated_at, dirty_work_id))` aggregate joined back to the pending project-scope rows, Top-N `LIMIT 2000`, then one `dirty_work_id IN (...)` update and one claim-state upsert per cycle. Pending rows whose `projection_component` is still NULL after lane repair (88,650 `projectScope.article.added` rows in the live snapshot) are completed as `orphan_missing_component` by rowid, 2,000 per cycle, once older than one hour. Search chunk batches are tiered by the DuckDB limit (8 up to 8,192 MiB, 16 up to 12,288 MiB, 32 above), overridable with `FORSKA_REVIEW_SERVING_SEARCH_REBUILD_CHUNK_BATCH_SIZE` (hard ceiling 64), clamped to the low-memory completed-chunk run budget, and capped at the RSS-governed rebuild chunk batch while the worker is under soft RSS pressure. A projector wake that is only blocked by queued foreground/append DuckDB work is admitted once per `FORSKA_REVIEW_SERVING_WAKE_STARVATION_MS` (30 s) with its normal `maxWakeMs`/batch/row budget, so the 6.4 GiB profile (0 of 22 wakes admitted in 17 minutes against the live clone) keeps draining dirty work without lifting the per-wake memory bound. The same 30 s rule also covers the budget path: in the 6.4 GiB profile the rebuild-chunk batch consumed the whole admission deadline every cycle and the wake ran with `maxWakeMs` 0 (`budget`), so a starved cycle now runs the backlog wake before the chunk batch (sharing the cycle deadline with the job-driven llmStatus pre-wake so the two wakes never exceed `maxWakeMs` together), keeps delta intake and the interval-gated cleanup on even while an activation drain is in progress, stamps the admission clock when that work ends, and only then runs the chunk batch if the normal (non-aging) yield decision admits it, so chunks stay skipped while foreground work is queued; an admitted sweep that claims nothing reports `idle` (logged as `projectorBlockedReason` `idle`) and counts as admitted, while only admission/budget blocks keep status `blocked` and leave the timer alone, so an idle backlog under sustained pressure is admitted once per window instead of every cycle. Batch and row budgets never collapse to 0 under RSS pressure (they floor at the low-memory 1/1), only the deadline did. Because the low-memory loop restarts after almost every cycle (`maxCompletedChunksPerRun` 16 with 8-chunk batches, plus RSS-cap restarts at 6.2 GiB), the heartbeat seeds `lastAdmittedWakeAtMs` once and carries it together with the rotation offset across bounded loop restarts exactly like `lastCleanupAtMs`; re-seeding per loop meant the 30 s never accumulated (soak #3: 14 of 14 cycles `budget`).
- Verification: In-memory DuckDB with 300,001 claim-state rows (180k pending high-water rows in 8 lanes, 120k completed) under `memory_limit=3GB`: each default coalesce cycle took 717-758 ms with about 1.3 MB of allocator use beyond the 150 MB table; project-fair claims took 18 ms (small project) to about 200 ms (64 rows from a 22k-row lane) and served the small project first. `bun test src/server/reviewServing/reviewServingDirtyWorkService.test.ts src/server/reviewServing/reviewServingProjectorService.test.ts src/server/workers/reviewServingProjectorWorker.test.ts src/server/utils/env.test.ts src/server/utils/reviewServingProjectorWorkerHeartbeat.test.ts`.
...
- Fix: Snapshot creation treats pre-copy checkpoint failure as fatal, logs it, and aborts the snapshot rather than returning a potentially stale checkpoint-only database copy. Operators should use the live owner/API path for current-DB checks until checkpoint memory pressure is resolved.
- Verification: `bun test src/server/utils/duckdbServiceReload.test.ts src/server/utils/duckdbServiceShutdown.test.ts src/server/utils/duckdbServiceMemoryLimit.test.ts src/server/utils/duckdbScriptAccess.test.ts`; `bun run test:dev-server:current-db`; current-DB live progress gate through the owner/API path.
