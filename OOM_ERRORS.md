...
- Fix: Low-memory migration runs avoid one large post-run checkpoint and temporarily lower the automatic checkpoint threshold to `1KiB`, incrementally persisting committed schema changes without leaving WAL-only `ALTER TABLE` state.
- Verification: `bun test src/db/migrateDuckdb.test.ts`; the interruption regression reopens a migrated database after `SIGKILL` without WAL replay.
