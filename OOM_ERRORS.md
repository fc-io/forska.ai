...
- Fix: Project selection is one aggregate statement (`MIN(updated_at)` of eligible rows and `MAX(updated_at)` of non-pending rows per project; never-served projects first, then least recently served, oldest eligible row, project id) followed by an `ORDER BY ... LIMIT 2048` Top-N window over that one project only, so memory is a 2048-row heap instead of a sort of the backlog. Coalescing is one bounded statement: a per-lane `MAX(struct_pack(high_water, updated_at, dirty_work_id))` aggregate joined back to pending NULL-range rows, Top-N `LIMIT 2000`, then one `dirty_work_id IN (...)` update and one claim-state upsert per cycle. Search chunk batches are tiered by the DuckDB limit (8 up to 8,192 MiB, 16 up to 12,288 MiB, 32 above), overridable with `FORSKA_REVIEW_SERVING_SEARCH_REBUILD_CHUNK_BATCH_SIZE`, and clamped to the low-memory completed-chunk run budget.
