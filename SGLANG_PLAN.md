...
- [x] Persist engine at the project level for historical clarity: add `engine` column to `projects` (enum/text like `'sglang' | 'vllm'`)
- [ ] backfill existing projects to `'vllm'`, and set `'sglang'` for new or migrated projects.
