...
- [ ] Inventory the smallest existing route, queue, dispatch, request-runtime,
  SQLite, outbox-import, repair, health, and status/read-model suites needed for
  the gate.
- [ ] Add `bun run test:judgment-workflow` as the named package script for the
  selected focused suites.
- [ ] Use deterministic stub-provider responses wherever judgment execution is
  required.
- [ ] Ensure the gate preserves the requested model, provider, thinking level,
  and content flags in all assertions.
- [ ] Document the command and its coverage in `TESTS.md`.
- [ ] Keep the gate isolated from the primary database, normal job storage, and
  network providers.
...
- [ ] Add `bun run test:judgment-workflow:e2e` as the named package script for
  the deterministic happy-path lifecycle test.
- [ ] Create a unique temporary DuckDB path for each test run. Verify the
  ...
  variable.
- [ ] Seed exactly one project, article, prompt, model, provider connection, and
  deterministic provider response.
- [ ] Create the job through the production route/service boundary.
- [ ] Start the job and run the production queue-fill path.
- [ ] Claim exactly one prompt through the production claim/dispatch path.
- [ ] Return one deterministic judgment using the configured request runtime.
- [ ] Assert the completed judgment is durable in the per-job SQLite store and
  pending export before import.
- [ ] Import the SQLite outbox and assert these independent boundaries in
...
- [ ] Read and assert the canonical `app.judgment` row immediately after the
  DuckDB commit using the complete identity key:
  `(articleId, promptId, modelId, useTitle, useAbstract, useFulltext,
  useFulltextNoImages)`.
- [ ] Assert the project review-serving read remains stale before its target
...
- [ ] Pause and drain through the actual status/storage sequence:
  ...
  intermediate state.
- [ ] Assert the per-job SQLite file remains present while `draining`, and that
  ...
  deletion.
- [ ] If lifecycle coverage includes local artifact deletion, invoke the
...
- [ ] Assert zero queued, claimed, running, judged-but-unexported, and
  unacknowledged rows remain.
- [ ] Make fixture cleanup unconditional and verify no background process or
...
- [ ] Name and document this as a component-lifecycle gate if it imports and
...
- [ ] Add `bun run test:judgment-workflow:topology` as a deterministic isolated
  ...
  orchestrator.
- [ ] Give the stack a unique disposable DuckDB path under a test-owned,
...
- [ ] Initialize the isolated database through the same startup migration path
  ...
  installation works.
- [ ] Configure a deterministic local network provider stub through the same
...
- [ ] Provision the topology fixture only through production HTTP boundaries or
...
- [ ] Create and start the job through HTTP, then poll public job/read-model
...
- [ ] Assert architectural evidence: only the DuckDB owner mutates DuckDB; the
  ...
  loop ticks.
- [ ] Add a contention subcase with two active jobs sharing one provider limit
...
- [ ] Include one bounded restart boundary: terminate the judge worker only
  ...
  absent.
- [ ] Stop the whole stack through its normal supervisor shutdown path and
...
- [ ] Add `bun run test:judgment-workflow:recovery` as the named command for
  failure, replay, and recovery boundary tests; keep these out of the
  happy-path `test:judgment-workflow:e2e` command.

- [ ] Replay an identical completion and assert idempotent canonical output.
- [ ] Inject failure before the DuckDB import commit and assert the SQLite
  outbox remains retryable and unacknowledged.
- [ ] Simulate a crash after DuckDB commit but before SQLite acknowledgement;
  rerun import and assert no duplicate canonical judgment.
- [ ] Expire a worker claim and assert stale-claim recovery preserves the work.
- [ ] Inject a transient SQLite lock and assert only the configured retry
  behavior occurs.
- [ ] Corrupt a job SQLite store and assert explicit quarantine and diagnostic
  evidence.
- [ ] Exhaust the configured retry budget and assert terminal failure without
  hidden re-admission.
- [ ] Test model/runtime mismatch and assert dispatch is rejected with the
  configured values intact.
- [ ] Test provider failure and assert the original failure is surfaced without
...
- [ ] Inventory and document the concrete job-detail and health response fields
...
- [ ] Assert the job detail route's actual `status`, `storageState`, queue,
...
- [ ] Assert the health route's actual counters and derived health badges at
...
- [ ] Assert the canonical `app.judgment` row is absent before import commit and
  present after it; do not incorrectly delay canonical durability until serving
  projection completion.
- [ ] Assert the project-facing review-serving route remains stale until its
  target dirty token completes, then exposes the imported judgment.
- [ ] Assert another `modelId` cannot satisfy the lookup.
- [ ] Assert each incompatible content configuration flag combination cannot
  satisfy the lookup.
- [ ] Assert route payloads distinguish `status` from `storageState`. Derive
...
- [ ] Add a synthetic fixture that can be started by Playwright without using a
  real provider or the primary database.
- [ ] Seed/create the synthetic job through the fixture/API production boundary
  before visiting `/admin/jobs`; do not treat `/admin/jobs` as a creation UI.
- [ ] Exercise the real browser flow: create the job from the projects grid if
...
- [ ] Exercise `/admin/jobs/:id`: verify lifecycle state, queue/judged progress,
...
- [ ] Pause and drain the job through the UI and verify the terminal state.
- [ ] Keep the existing mocked telemetry tests for loading, error, and
  presentation edge cases.
- [ ] Run the lifecycle smoke in the normal browser/web flow.
- [ ] Treat the existing Playwright `dev-single` composition as a UI/API
...
- [ ] Separately scope a future automated desktop harness, fixture startup and
...
- [ ] Ensure the fixture stops all servers/workers and removes temporary state.
...
- [ ] Add the exact operator-only command
  ...
  model request.
- [ ] During implementation, verify against current official provider
...
- [ ] Provision the Codex provider connection and selectable model through the
...
- [ ] Before creating the job, validate only free/local prerequisites: explicit
...
- [ ] Run a small, fixed corpus of versioned snapshots of real articles and one
...
- [ ] Launch the real Codex smoke through the same isolated production
...
- [ ] Judge each article using title and abstract only. Assert
...
- [ ] Include representative title-and-abstract cases in the fixed corpus, such
...
- [ ] Bound spend and nondeterminism explicitly: pin the reviewed corpus and one
...
- [ ] Assert the dispatched provider, model, thinking level, and content flags
  exactly match the requested configuration.
- [ ] Assert the response completes the same import/materialization/read path as
...
- [ ] Surface provider admission and request failures unchanged.
- [ ] Add a cost/network warning and keep the command out of normal PR and
  default local gates.
- [ ] Document required environment variables and cleanup behavior in
  `TESTS.md`.
...
- [ ] Add the focused backend gate, happy-path lifecycle command, failure and
  ...
  smoke to `TESTS.md`.
- [ ] State which gates are required for PRs and which are operator-only.
- [ ] Record fixture isolation, temporary storage, and cleanup guarantees.
- [ ] Record the exact benchmark-critical settings asserted by each gate.
- [ ] Note touched layers in the eventual commit/PR: server, client, database
...
- [ ] `bun run lint` passes without fixing unrelated lint issues.
- [ ] `bun run test:judgment-workflow` passes over the targeted route, queue,
  dispatch, SQLite, request-runtime, outbox, repair, health, and read-model
  suites.
- [ ] `bun run test:judgment-workflow:e2e` passes and leaves no unresolved
  queue, outbox, lease, import, or materialization acknowledgement state.
- [ ] `bun run test:judgment-workflow:topology` launches the isolated production
...
- [ ] The opt-in real-provider smoke is documented but is not required for a PR;
  ...
  policy.
- [ ] Test teardown leaves no server or worker processes, temporary DuckDB
  ...
  deletion works.
- [ ] `TESTS.md` identifies the end-to-end chain and clearly distinguishes fast
  PR gates from browser/desktop and operator-only provider gates.
- [ ] `bun run test:bun` passes as the full backend pre-merge gate.
