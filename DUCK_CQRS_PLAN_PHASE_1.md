...
Status: Phase 1 foundation is implemented, with one audit fix applied for raw
JSON placement in hot serving tables.

- Schema foundation evidence: `src/db/duckdbMigrations/0097_reviewServingV4Foundation.sql` creates the required delta ledgers, reconciliation cursors, dirty-work state, projector watermarks/manifests, selected-import snapshots, snapshot manifests/pins, job tables, retention metadata, and `_v4` serving marts. Follow-up migrations `0098` through `0106` tighten payload ordering, count/list-mode scoping, detail/filter/facet discriminators, queue identity, article display metadata, and the forward-schema removal of raw `source_metadata` from hot V4 article rows.
- Schema test evidence: `src/server/reviewServing/reviewServingSchema.test.ts` proves the Phase 1 table set exists, read-contract physical tables exist, non-job cursor/sort columns exist, job contracts stay on `updated_at`/`job_id`, prompt-preview order columns exist, detail/filter-option/facet/count discriminator columns exist, manifest reads use `snapshot_status`, list metadata columns exist, common delta envelopes exist, and hot serving tables do not include raw `source_metadata JSON`.
- Contract evidence: `src/server/reviewServing/reviewServingReadContracts.ts` declares `_v4` read-contract physical tables, per-contract `timeoutMs`, `searchMode`, budgets, freshness behavior, workload class, cursor fields, and route inventory. `src/server/reviewServing/reviewServingReadContracts.test.ts` verifies contract coverage, static fields, search-mode declarations, route inventory, and mounted/internal classification.
- Admission evidence: `src/server/reviewServing/reviewServingAdmission.ts` maps admitted read contracts into `DuckdbWorkloadContext` with `routeOrJobKey`, workload class, project ID, row/byte budgets, temp-spill policy, `timeoutMs`, `searchMode`, and fallback intent. `src/server/reviewServing/reviewServingAdmission.test.ts` proves unregistered work, workload-class mismatches, over-budget work, invalid budget values, synchronous substring search, mismatched search modes, unsupported count shapes, and stale-fallback behavior are handled before DuckDB execution.
- Runtime evidence: `src/server/utils/duckdbService.ts` accepts optional workload context, records contextual metrics, and rejects contextual work that exceeds row, byte, temp-spill, or timeout budgets without embedding product-specific SQL. `src/server/utils/duckdbServiceWorkloadContext.test.ts` proves over-budget contextual work records metrics and is rejected.
- Wrapper evidence: `src/server/services/appDatabaseService.ts`, `src/server/services/readOnlyDuckdbService.ts`, `src/server/services/appReadOnlyDatabaseService.ts`, and `src/services/olap/duckdbRunner.ts` thread optional workload context. `src/server/services/readOnlyDuckdbServiceWorkloadContext.test.ts` proves read-only contextual metrics and app read-only forwarding.
- Fixed issue: `mart.review_article_serving_v4` and `mart.review_article_display_patch_v4` previously carried raw `source_metadata JSON` in the forward schema, contradicting the Phase 1 hot-schema rule. The fix removes those columns from the forward schema and projector writes, keeps `source_metadata` in `mart.review_article_serving_payload_v4`, and left-joins that capped payload companion for article-row reads in `src/server/reviewServing/reviewServingSql.ts`. The live migration `0106` is a marker no-op because DuckDB rejected an in-place drop/rebuild through the migration runner due internal table dependencies; existing already-applied local databases may retain an unused physical column, but current code no longer reads or writes it on hot rows.
- Stale assumption: the Phase 1 cut line says product routes are not switched, but current `HEAD` already includes later-phase mounted route migrations in `reviewServingReadContractRouteInventory`. This audit treats that as a later-phase state on latest main, not as Phase 1 evidence. Phase 1-dependent later work should rely on the schema/admission/runtime foundation above, not on the historical “routes not switched” condition being true at current `HEAD`.
- Remaining gaps for Phase 1: no code or forward-schema gaps remain after the raw hot-row metadata fix. Already-applied local databases can retain the now-unused hot-row `source_metadata` column because DuckDB blocked the in-place migration drop; if later work requires physical cleanup of previously migrated local files, do it as an explicit table rebuild outside the generic migration runner.
