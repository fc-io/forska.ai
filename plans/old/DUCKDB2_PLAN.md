...
- [x] Inventory remaining Better Auth/schema/tooling leftovers: `auth-schema.ts`, `src/seedAuth.ts`, `src/seed.ts`, `scripts/resetPassword.ts`, old schema-tooling configs, `src/server/utils/env.ts`, `package.json`, and old remote-run docs.
