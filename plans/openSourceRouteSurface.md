...
Status: code-backed current-state baseline. `src/server/routes/routeSurfaceInventory.ts` now records the mounted listener/proxy entrypoints and every currently mounted API route in `runtimeReadyRoutes`, `DuckdbOwnerConnectionsRoutes`, `JudgmentDispatchTelemetryRoutes`, and `getProductApiRoutes()`.
...
| `src/server/routes/publicRouteSurfaceGate.ts` | Public local API gate | Runs before the owner proxy and returns 404 for public `/api/*` routes classified as internal runtime, local diagnostics/status, maintenance/debug, or remove-before-release. Owner-private RPC paths and internal runtime peer heartbeats remain available for split-runtime coordination. | `operator/infra` | Keep. Local operator routes require explicit `FORSKA_EXPOSE_LOCAL_OPERATOR_API=true` exposure. |
| `src/server/routes/ApiProxyRoutes.ts` | `onRequest` proxy middleware | Proxies owner-dependent API requests to the current DuckDB owner when the current role is an owner-proxy. Unknown `/api/*` routes remain proxy/fail-closed candidates. | `operator/infra` | Keep internal/local-only. Current mounted routes are classified by `routeSurfaceInventory.ts`. |
| `src/server/routes/apiRouteClassification.ts` | Route classification helper | Uses `routeSurfaceInventory.ts` to classify current mounted API routes for owner-proxy behavior, while leaving unknown `/api/*` paths as `unclassified` and fail-closed. | `operator/infra` | Keep. This is the owner/proxy classifier, not the final public supported-local-API contract. |
...
| --- | --- | --- | --- | --- |
| `runtimeReadyRoutes.ts` | `GET /api/runtime/ready`, `GET /api/runtime/state` | Runtime readiness, role, capabilities, PID, Bun request-limit state. | `local-only`, `operator/infra` | Keep readiness public-local. Gate runtime state/status on the public API by default. |
| `DuckdbOwnerConnectionsRoutes.ts` | `GET /api/duckdb_owner_connections`, `POST /api/duckdb_owner_connections/heartbeat` | DuckDB-owner registry, heartbeats, proxy metadata, mart refresh throughput. | `operator/infra` | Gate public status by default. Allow internal peer heartbeats for split-runtime coordination. |
| `JudgmentDispatchTelemetryRoutes.ts` | `GET /api/admin/judgment-dispatch-runtime/:jobId` | Local judgment dispatch telemetry and provider capacity diagnostics. | `admin/debug`, `operator/infra` | Gated on the public API by default. |
| `AdminInvestigateRoutes.ts` | `GET /api/admin/duckdb-append-metrics`, `GET /api/admin/maintenance-runtime-diagnostics`, `GET /api/admin/worker-runtime-diagnostics`, `GET/POST /api/admin/project-mart-large-rebuild-*`, `GET /api/admin/list-prompts-with-types`, `POST /api/admin/delete-unexpected-answers`, `POST /api/admin/auto-sync-all-unexpected-answers`, `GET /api/admin/auto-sync-all-progress`, `GET /api/admin/investigate-unexpected-answers` | Runtime diagnostics, mart rebuild control, prompt/judgment cleanup and investigation. | `admin/debug`, `operator/infra` | Gated on the public API by default; remove or keep as explicit local operator API before release. |
...
| `ProjectsRoutes.ts` and nested review routes | `GET/POST/PATCH/DELETE /api/projects*`, `GET /api/projects-without-jobs`, `GET /api/projects/:id/access`, `POST /api/articlesreviews*`, `GET /api/articlesreviewsfilters`, `GET /api/articlesreviewshumanfilters`, `POST /api/projectsreview`, `POST /api/projectsreviewswarnings` | Project CRUD, review lists/details/filters/warnings, prompt and model edits, archive/unarchive/clone. | mostly `public product`, `local-only` | Keep product routes, including review warning reads used by the review UI. |
| `projectsRoutesPostDeleteArchived.ts` | `POST /api/projects/delete-archived` | Requests archived project cleanup. | `admin/debug`, `operator/infra` | Gated on the public API by default. |
...
| `DuckdbStudioRoutes.ts` | `POST /api/duckdbStudioSnapshots` | Creates DuckDB snapshots and requires owner role. | `admin/debug`, `operator/infra` | Gated on the public API by default. Not public product. |
| `TokensRoutes.ts` | `POST /api/tokens/usage`, `GET /api/tokens`, `GET /api/tokens/largest-*`, `POST /api/tokens/timeline*`, `POST /api/tokens/failed-requests`, `GET /api/tokens/failed-requests/:id` | Token usage analytics and failed request details. | mixed: `operator/infra`, `local diagnostics`, sensitive details | Aggregate/status routes are gated on the public API by default. Failed request details remain sensitive and need redaction/doc decisions. |
...
| `LlmStatusRoutes.ts` | `GET /api/llmstatus` | LLM runtime metrics from local DB used by the shell and status page. | `public product`, `local-only` | Keep as a supported local product read while the shell polls it. |
| `NvidiaSmiRoutes.ts` | `GET /api/nvidiasmi` | GPU metrics from local DB. | `local-only`, `operator/infra` | Gated on the public API by default. |
...
| Owner RPC mirror | `/__duckdb-owner-rpc/**` | Mirrors product API routes under `duckdbOwnerPrivateApiPrefix` when the current role mounts the owner RPC API. | `operator/infra` | Keep internal only. It remains loopback-only and is skipped by the public route gate for split-runtime internals. |

## Current Blockers

| Blocker | Why it matters | Suggested next action |
| --- | --- | --- |
| API and app server bind hosts are explicit loopback defaults. | The release principle requires loopback-only defaults or a documented exception. | Keep `routeSurfaceInventory.test.ts` as a regression guardrail when listener/runtime wiring changes. |
| Admin/debug/operator routes are still mounted but public-gated by default. | This avoids accidental public-local support while preserving explicit local operator workflows. | Decide which gated routes should be removed entirely before release versus kept behind `FORSKA_EXPOSE_LOCAL_OPERATOR_API=true`. |
| `/api/*` app-server proxy forwards any API path. | A catch-all proxy makes the actual exposed surface broader than docs unless the API server itself is tightly classified. | Keep with the current inventory and route tests; final public docs must list only supported local API routes. |
| Several routes can expose sensitive local state or content. | Provider secrets, failed request details, runtime assets, uploaded PDFs, PDF fetches, Codex login, exports, imports, token/request traces, and settings need public-release decisions. | Review and document each sensitive path before release. |
...
| FHIR/EHR patient import is still mounted and marked `remove-before-release` in the inventory. | Patient-related import surface is high-risk for public release and docs. | It is public-gated by default; remove before release unless explicitly justified and documented. |
...
4. Use `routeSurfaceInventory.ts` and `routeSurfaceInventory.test.ts` as the current route-review baseline until the final supported local API manifest is ready.
...
| --- | --- |
| Manual verify route coverage | `routeSurfaceInventory.ts` covers `src/server/serverMain.ts`, `src/appServerMain.ts`, `src/desktop/index.ts`, `runtimeReadyRoutes`, `DuckdbOwnerConnectionsRoutes`, `JudgmentDispatchTelemetryRoutes`, `getProductApiRoutes()`, route classification, proxy entrypoints, owner-private RPC, and cron-mounted Elysia plugins. |
