...
Status: draft current-state baseline. This document records the route and network surface that must be resolved before an open-source release. It does not change runtime behavior.
...
| `JudgmentsRoutes.ts` | `GET /api/judgments/model` | Gets or creates a default model/provider record from query defaults. Called by project create/edit and Covidence import flows. | `unknown/blocker` | Replace with explicit product setup or ensure route. Current GET can mutate state and appears legacy. |
...
| `ProjectsAddArticlesRoutes.ts` | `POST /api/projects/add_articles_by_filter`, `POST /api/projects/add_artilces_by_ids` | Bulk add articles to projects by filter or id. | `public product`, `dead/remove` for typo alias | Keep needed behavior. Replace or remove typo path before public release if not required. |
...
| `ImportRoutes.ts` | `GET /api/import-routes`, `GET /api/importroutes` | Import route list. | `public product`, `dead/remove` for legacy alias | Keep canonical `/api/import-routes`. Review/remove legacy `/api/importroutes` if unused. |
...
| Legacy or questionable paths remain mounted. | Typos, aliases, and GET-with-side-effect routes become public API once open-sourced. | Decide on `/api/projects/add_artilces_by_ids`, `/api/importroutes`, `/api/judgments/model`, and provider-admission alias paths. |
