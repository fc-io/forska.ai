...
- Full history contains substantial old private infra and remote-ops material in docs, scripts, Docker/compose files, sbatch files, and backup/sync helpers. This confirms the preferred release path in `OS_IT_PLAN.md`: publish from a clean audited snapshot, not the existing history.
