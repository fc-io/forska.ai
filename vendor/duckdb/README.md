...
- `disabled_optimizers=cte_inlining,statistics_propagation`: CTE inlining can raise native `Vector::Reference` errors in real LLM/human status projections and review-page queries. Separately, replaying an alpha WAL over a stable 1.5.1 checkpoint can merge legacy 8-byte string-statistic prefixes with newer 12-byte bounds incorrectly; statistics propagation then replaces valid ID lookups with `EMPTY_RESULT`. The original rows are intact: stable 1.5.1 and the configured alpha return them correctly from identical DB/WAL bytes. Only these two passes are disabled; scan-level pruning, the remaining optimizers, and `delim_join_as_cte` retain their defaults. Disabling `delim_join_as_cte` alone does not fix both native CTE failures.
...
Run `bun test src/server/utils/duckdbEngineCompatibility.statistics.test.ts` for the separately pinned string-statistics negative control and all managed/read-only reader paths. Its tiny public-data fixture preserves a stable checkpoint and a committed alpha WAL that deletes one row and inserts another. Exact retained and inserted IDs, deleted IDs, range filters, joins, all rows, and checkpoint/fresh-reopen results must match. Distribution verification repeats the real predicates with the installed or copied native package. The root cause is the `StringStats::MergeStats` handling of truncated bounds introduced with [DuckDB #22692](https://github.com/duckdb/duckdb/pull/22692); the pinned source compares different-length bounds directly when merging. Disabling propagation prevents those incorrect bounds from proving a false empty result; it does not rewrite user data, delete WAL, or disable scan pruning. This setting may change query plans and cost, so retain the unchanged-memory real-progress gate. Remove it only once an official engine passes the same fixture with statistics propagation enabled, including checkpoint/reopen and every predicate/reader path. A successful `ANALYZE`, recovery, or WAL deletion alone is not a root-cause fix.
