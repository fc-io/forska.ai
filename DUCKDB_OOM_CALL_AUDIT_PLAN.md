...
- [~] **A30.** Extend admission/diagnostic evidence for unchecked detail,
      ...
      Remaining blockers: detail, warning/health, and export still document
      residual source reads or metadata gaps; comparison has no separate V4
...
- [~] **A34.** Retire `duckdbOlap.ts` raw fallback branches after the last product
  ...
  `duckdbOlap.test.ts` and the product detail, diagnostic, export, and
  comparison route rows above still list residual blockers; prompt preview no
  longer contributes a raw-fallback blocker.
...
- [ ] **A40.** Retire dirty materialization and mart refresh as legacy serving
      ...
      from the product review warnings route/UI. A40 remains partial because
      `getDuckdbMartMaintenanceService.ts`, `projectMartRefreshWorker.ts`,
      dirty-refresh producers, review detail diagnostics, and large-rebuild
      workers still retain legacy serving paths.
