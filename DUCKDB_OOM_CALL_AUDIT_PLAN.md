...
- [ ] **A40.** Retire dirty materialization and mart refresh as legacy serving
      ...
      existing legacy scope rows. Current A40 script cleanup removes
      the direct `runProjectMartRefreshWorker.ts` and
      `runProjectMartRefreshWorkerOnce.ts` CLI entry points; the isolated
      recovery script now routes any dirty-refresh claim into a V4 rebuild
      request. Current A40 worker cleanup deletes `projectMartRefreshWorker.ts`
...
| [~]    | Dirty materialization                   | Deleted `projectMartDirtyMaterializationService.ts`, `projectMartDirtyRefreshStateService.ts`, `projectMartDirtyRefreshQuarantineBarrier.ts`                                                                                        | Owner/background writer plus V4 queue/projection state  | Partial       | Writer        | Partial       | true - need further changes | Complete for the old materialization service: whole-project dirty marks write bounded `project_mart_refresh_article_state` rows directly, the direct worker/package entry points are gone, the isolated recovery CLI converts dirty-refresh claims into V4 `review_rebuild_request` rows, and `projectMartDirtyMaterializationService.ts` plus its service test are deleted. Partial only because `projectMartDirtyRefreshStateService.ts` and `projectMartDirtyRefreshQuarantineBarrier.ts` remain as legacy ledger/recovery/quarantine state until the remaining mart refresh service is deleted. |
| [x]    | Mart refresh worker                     | Deleted `src/server/workers/projectMartRefreshWorker.ts`, deleted `getDuckdbMartMaintenanceService.ts`, deleted `reviewAnswerDictionaryStabilityService.ts`                                                                        | Retired legacy writer                                   | N/A           | Retired       | N/A           | true - deleted              | Complete: the worker loop module and tests, package scripts, heartbeat loops, and direct CLI entry points are gone; the isolated recovery CLI now converts dirty-refresh claims into V4 rebuild requests instead of running incremental legacy mart refresh; the DuckDB-owner overview route no longer imports legacy mart refresh throughput; the orphaned review-answer dictionary stability service/test are deleted; and `getDuckdbMartMaintenanceService.ts` plus its test are deleted after the last production caller disappeared. Evidence: static guards keep the retired files absent, route/import tests no longer need this service, and remaining large-rebuild executor tests no longer compare against the old incremental refresh helper.                                                                                                                     |
