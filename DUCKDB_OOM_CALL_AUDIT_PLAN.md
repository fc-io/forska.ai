...
- [~] **A50.** Keep review UI article queries gated on V4 warning/readiness data
  ...
  warning-query failure as article-load permission. Still partial because final
  A52 readiness-state wiring remains.
...
- [~] **A53.** Rewire admin UI away from legacy mart large-rebuild controls and
      prove every admin surface is diagnostic-only or V4-backed. Current static
      and route tests prove the legacy large-rebuild page/link is gone and that
      dirty-materialization/large-rebuild mutation routes return retired
      responses. Still partial until A49 inventories every admin page/API, not
      only the legacy mart controls.
