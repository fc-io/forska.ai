...
- [ ] **A40.** Retire dirty materialization and mart refresh as legacy serving
      ...
      maintenance. Current A40 guard pass: legacy dirty materialization and mart
      refresh remain quarantined to background bounded claims and are forbidden
      from writing V4 snapshot/projector state, but retirement is blocked while
      `getDuckdbMartMaintenanceService.ts` still writes legacy mart serving
      tables used by current product flows.
