...
- [x] **A6.** Add a broad normal-foreground SQL guard for forbidden raw shapes:
      ...
      sort/extraction.
- [x] **A7.** Tighten low-level DuckDB execution so normal foreground work with
  missing `DuckdbWorkloadContext` is rejected before connection acquisition,
  while explicit owner/background/admin/test scopes remain allowlisted. The
...
- [x] **A10.** Add runtime-role assertions proving API role cannot acquire
  ...
  Current evidence covers `duckdbService.ts`, `appDatabaseService.ts`, and
  read-only wrappers; `getAppQueryService.test.ts` now proves app-query
  residual source reads carry explicit owner source workload contexts, while
  route proxy/inventory guardrails keep those source reads owner-routed.
- [x] **A11.** Implement the missing-workload-context rejection from A7 with tests
  beside `duckdbServiceWorkloadContext.test.ts`, including allowed
  maintenance/admin/test paths. Current tests cover default API-role rejection,
  explicit opt-out behavior, `appDatabaseService.ts` wrapper rejection before
  DuckDB instance creation, owner/background/maintenance allowance, read-only
  workload forwarding, and `getAppQueryService.ts` owner-source contexts.
...
- [x] **A21.** Migrate comparison project reads to complete bounded serving
  contracts/admission and keep source writes owner-routed. Current evidence:
  `comparisonProjectServingRouteContracts.ts` classifies every comparison API
  route as serving-read, owner source metadata/validation, or owner write plus
  serving rebuild/cleanup. `ComparisonProjectsRoutes.servingContract.test.ts`
  guards the bounded active-generation serving reads, and
  `comparisonProjectServingRouteContracts.test.ts` guards full route coverage
  plus active-generation contracts for comparison stats, judgment rows/counts,
  export, conflict-resolution export, and conflict-resolution import analysis.
  Existing-project import analysis/commit now target-match against active
  `mart.comparison_article_serving` and
  `mart.comparison_article_identifier_serving` rows and fail closed without an
  active comparison serving generation; create-from-project preview remains
  explicit owner-routed source validation because it runs before a comparison
...
- [x] **A30.** Extend admission/diagnostic evidence for unchecked detail,
      prompt-preview, warning/health, comparison, and
      human-assessment contracts before marking those product rows migrated.
      Current evidence is in `reviewServingReadContracts.test.ts`,
      ...
      `comparisonProjectServingRouteContracts.test.ts` now adds comparison route
      contract coverage, including serving-owned target matching for existing
...
- [x] **A40.** Retire dirty materialization and mart refresh as legacy serving
      ...
      existing legacy scope rows. Current A40 script cleanup removes the direct
      `runProjectMartRefreshWorker.ts`, `runProjectMartRefreshWorkerOnce.ts`,
      and `runProjectMartRefreshWorkerOnceIsolated.ts` CLI entry points; the
      remaining dirty-refresh-claim recovery script is explicitly named
      `requestReviewServingForDirtyRefreshClaim.ts` and routes any claim into a
      V4 rebuild request. Current A40 worker cleanup deletes `projectMartRefreshWorker.ts`
      and the old worker tests, current dirty-materialization cleanup deletes
      `projectMartDirtyMaterializationService.ts` plus its service test, and the
      orphaned review-answer dictionary stability service/test are deleted
      instead of preserving another legacy mart pruning path, and the final
      test-only `getDuckdbMartMaintenanceService.ts` dependency/service/test are
      deleted. Current legacy script cleanup also removes neutral package command
      names for dirty-refresh diagnostics/recovery/quarantine; the remaining
      package-exposed commands are explicitly `legacy-*`, and mutating recovery,
      quarantine, and unquarantine scripts require the legacy dirty-refresh admin
      acknowledgement before touching old state. Final A40 cleanup removes all
      production app/agent/server imports of `projectMartDirtyRefreshStateService`;
      normal source, project, prompt, import, LLM judgment, and human judgment
      write paths now rely on V4 project-scope, review-config, LLM, and human
      deltas instead of writing `project_mart_refresh_state` or
      `project_mart_refresh_article_state`. `projectMartDirtyRefreshStateService.ts`
      remains only as legacy recovery/quarantine/cutover machinery until the
      old ledger tables can be dropped.
- [x] **A41.** Convert large rebuild services into V4 chunk-manifest/backfill
      ...
      legacy large-rebuild fallback. It also renames the remaining package
      exposed single-project request command from legacy "project large rebuild"
      wording to explicit review-serving V4 rebuild wording, and deletes the
      unused mart-refresh and large-rebuild heartbeat loop modules now that
      startup is cut over to the V4 projector worker. Current A41 route/UI
      cleanup removes legacy large-rebuild storage-drain projections from the
      judgment-job detail API and admin job page, and `rebuild2Cutover.ts`
      converts legacy large-rebuild rows into V4 rebuild requests instead of
      re-staging `project_mart_large_rebuild_state`; `inspectDirtyRefreshRisk.ts`
      now reports old large-rebuild rows as retired legacy state with a V4
      rebuild recommendation instead of planning legacy large-rebuild work. The
      internal `projectMartLargeRebuild*` executor/runner/cycles/progress/state
      services and runtime-metrics module are deleted after the final non-test
      callers disappeared; dirty-refresh claiming now ignores retired
      large-rebuild rows instead of treating them as live handoff blockers.
...
- [x] Low-level foreground DuckDB execution rejects missing workload context
  outside explicit owner/background/admin/test scopes. Current enforcement:
  ...
  acquisition by default; `FORSKA_ENFORCE_DUCKDB_WORKLOAD_CONTEXT=false`
  remains as an explicit rollout/debug opt-out. Evidence:
  `duckdbServiceWorkloadContext.test.ts` covers default low-level API rejection,
  opt-out behavior, `appDatabaseService.ts` wrapper rejection without creating a
  DuckDB instance, and owner/background/maintenance allowance; `getAppQueryService.test.ts`
  covers owner-source app-query contexts; and read-only workload forwarding is
  covered by `readOnlyDuckdbServiceWorkloadContext.test.ts`.
...
| [x]    | Runtime role and owner registry | `src/server/utils/serverRuntimeRole.ts`, `src/server/utils/duckdbOwnerConnections.ts`, `src/server/utils/duckdbOwnerConnectionHeartbeat.ts`, `src/server/utils/duckdbOwnerLease.ts`, `src/server/utils/runtimeCutover.ts` | Owner/background writer                                         | N/A           | Control       | N/A           | false                       | Current: owner election, heartbeat, cutover compatibility, and failover registry code/tests exist. `duckdbServiceWorkloadContext.test.ts` proves API-role foreground work through the low-level service and `appDatabaseService.ts` wrapper fails before DuckDB instance creation when the workload-context guard is enabled; `getAppQueryService.test.ts` proves app-query residual source reads carry explicit owner source workload contexts; route proxy/inventory guardrails keep owner-dependent source reads off API-role local DuckDB ownership. |
| [x]    | Low-level DuckDB service        | `src/server/utils/duckdbService.ts`, `src/server/services/appDatabaseService.ts`                                                                                                                                          | Owner/background writer                                         | Complete      | Reader/writer | Complete      | false                       | Complete for the low-level guard scope: workload metrics, budgets, and `DuckdbWorkloadContext` are implemented/tested in `duckdbServiceWorkloadContext.test.ts`. API-role foreground `mainQuery`/`mainStatement`/`transaction` without context is rejected before connection acquisition by default while owner/background/maintenance scopes remain allowed; `FORSKA_ENFORCE_DUCKDB_WORKLOAD_CONTEXT=false` is the explicit rollout/debug opt-out. `appDatabaseService.ts` `queryJson`/`run`/`transaction` inherit that guard without creating a DuckDB instance, app-query residual source reads pass explicit owner source contexts, and read-only helpers forward workload contexts without using owner-capable writer APIs. |
...
| [x]    | App query wrapper               | `src/server/services/getAppQueryService.ts`, `src/server/services/appQueryServiceCore.ts`                                                                                                                                 | Owner-routed source access / residual allowlist                 | Partial       | Reader        | N/A           | false                       | Current classification: source metadata/detail reads are owner-routed by API proxying, `reviewServingResidualReadAllowlist.ts` lists each product-facing `appQueryServiceCore.ts` method with purpose, cap, workload class, and migration target, and `getAppQueryService.test.ts` proves the wrapper passes explicit `owner.sourceAppQuery` workload contexts for project config, prompt, hydration, and full-article residual reads. Planned: finish V4 detail/payload migration so residual app-query reads can be removed. |
...
| [x]    | Comparison projects                        | `src/server/routes/ComparisonProjectsRoutes.ts`, `src/server/routes/comparisonProjectsRoutes/*`, `src/server/services/comparisonProjectServing*.ts`                                                                | Owner-routed source access / serving migration          | Complete      | Reader/writer | Complete      | false                       | Complete for the audited comparison route scope: API role proxies/fails closed through `routeSurfaceInventory.ts`; normal judgment page/count/stats/export reads use active-generation serving helpers; count, export, and conflict-resolution export fail closed without active comparison serving generation; conflict-resolution export reads article identity plus canonical identifiers from `mart.comparison_article_serving` and `mart.comparison_article_identifier_serving`; and existing-project conflict-resolution import analysis/commit target-match against those active serving tables instead of `app.article`, `app.article_identifier`, or source project scopes. `ComparisonProjectsRoutes.servingContract.test.ts` guards bounded serving reads plus owner-routed source writes/rebuild cleanup, `ComparisonProjectsRoutes.rollback.test.ts` covers serving-backed import analyze/commit behavior, and `comparisonProjectServingRouteContracts.test.ts` proves every comparison API route has an explicit serving/source/write handling class plus migration target. Create-from-project import preview remains explicit owner-routed source validation because it runs before a comparison serving generation exists. |
...
| [x]    | Dirty materialization                   | Deleted `projectMartDirtyMaterializationService.ts`; legacy recovery state remains in `projectMartDirtyRefreshStateService.ts` and quarantine/cutover scripts                                                                            | Retired legacy recovery adapter                         | N/A           | Maintenance writer | Partial       | true - need further changes | Complete for A40 product/runtime scope: whole-project dirty marks no longer use `project_mart_dirty_materialization_state`, the direct worker/package entry points are gone, `requestReviewServingForDirtyRefreshClaim.ts` converts dirty-refresh claims into V4 `review_rebuild_request` rows, and `projectMartDirtyMaterializationService.ts` plus its service test are deleted. Production app/agent/server code no longer imports `projectMartDirtyRefreshStateService`; normal source, project, prompt, import, LLM judgment, and human judgment writes rely on V4 project-scope, review-config, LLM, and human deltas instead of writing the legacy dirty ledger. The remaining package-exposed commands are explicitly `legacy-*`, mutating recovery/quarantine/unquarantine scripts require `legacy-dirty-refresh` admin acknowledgement, and the service stays only for recovery/quarantine/cutover until old ledger tables are dropped. |
| [x]    | Mart refresh worker                     | Deleted `src/server/workers/projectMartRefreshWorker.ts`, deleted `getDuckdbMartMaintenanceService.ts`, deleted `reviewAnswerDictionaryStabilityService.ts`                                                                        | Retired legacy writer                                   | N/A           | Retired       | N/A           | true - deleted              | Complete: the worker loop module and tests, package scripts, heartbeat loops, and direct CLI entry points are gone; `requestReviewServingForDirtyRefreshClaim.ts` converts dirty-refresh claims into V4 rebuild requests instead of running incremental legacy mart refresh; the DuckDB-owner overview route no longer imports legacy mart refresh throughput; the orphaned review-answer dictionary stability service/test are deleted; and `getDuckdbMartMaintenanceService.ts` plus its test are deleted after the last production caller disappeared. Evidence: static guards keep the retired files absent and route/import tests no longer need this service.                                                                                                                                   |
| [x]    | Large rebuild services                  | Deleted `projectMartLargeRebuildExecutor.ts`, `projectMartLargeRebuildRunner.ts`, `projectMartLargeRebuildCyclesService.ts`, `projectMartLargeRebuildStateService.ts`, `projectMartLargeRebuildProgressService.ts`, `projectMartLargeRebuildRuntimeMetrics.ts` | Retired legacy writer                                   | N/A           | Retired       | N/A           | true - deleted              | Complete: the legacy large-rebuild worker scripts/package commands, heartbeat loop, admin run/pause/resume/note/status controls, admin client page, runtime diagnostics, settings tuning controls, archive-service state-service call, and storage-drain projections are removed or retired. Recovery/cutover scripts now convert old large-rebuild rows into V4 `review_rebuild_request` work or label them retired diagnostic state. After the final non-test callers disappeared, the internal executor/runner/cycles/progress/state services, runtime metrics module, and dedicated tests were deleted; `projectMartDirtyRefreshStateService.ts` no longer skips claims because an old `project_mart_large_rebuild_state` row exists. The table/migrations remain only as legacy schema/recovery residue until a later schema-drop migration. |
| [x]    | Comparison serving builders             | `comparisonProjectServingGenerationService.ts`, `comparisonProjectServingRebuildService.ts`, `comparisonProjectServingRollupBuilder.ts`, `comparisonProjectServingCellBuilder.ts`, `comparisonProjectServingInvalidationService.ts` | Owner/background writer / serving migration             | Yes           | Writer        | Complete      | false                       | Complete for A42/A21 scope: comparison builders use background DB methods, generation promotion state, and materialize article identity fields plus `mart.comparison_article_identifier_serving` so export and existing-project conflict-resolution import target matching can share bounded serving identities. Generation cleanup deletes the identifier serving table alongside article/cell/filter tables, and rollup/tests prove scoped external IDs, DOI/PMID fields, and identifier rows are projected. Default generation, rebuild, cell, rollup, and invalidation dependencies now pass explicit `owner.comparisonServing` DuckDB workload contexts via `comparisonProjectServingWorkloadContext.ts`; `comparisonProjectServingDuckdbAccess.test.ts` guards those labels. `comparisonProjectServingRouteContracts.ts` supplies the V4-style route/admission registry, while source/write route classification remains covered by the comparison product-route row. |
...
| [x]    | Operator scripts                        | `scripts/*.ts` that call DB services, `dbBackup.ts`, `dbQuerySnapshot.ts`, `duckdbCheckpoint.ts`, recovery/rebuild/request scripts                                                                                                  | Admin/debug/tool allowlist                              | N/A           | Admin/tool    | N/A           | false                       | Current package-exposed DB script surface is allowlisted in `operatorScriptDuckdbAccess.test.ts`. Snapshot tools (`dbBackup.ts`, `dbQuerySnapshot.ts`, `dbStudio.ts`) use `createDuckdbSnapshotForCli`; snapshot query/studio are read-only. Maintenance commands for checkpoint, legacy dirty-refresh diagnostics/recovery/quarantine, V4 rebuild/repair requests, rebuild2 cutover, archived cleanup, and source/PPR backfills use `withDuckdbMaintenanceAccess` and explicit maintenance workload contexts where they issue owner DB work. Package commands no longer expose legacy mart refresh or legacy large-rebuild workers; project and all-project rebuild commands now use explicit review-serving V4 naming (`db:duck:request-review-serving-project-rebuild`, `db:duck:request-review-serving-all-projects-rebuild`) instead of project-mart large-rebuild wording. Remaining dirty-refresh package commands use explicit `db:duck:legacy-*` names, and mutating dirty-refresh recovery/quarantine scripts require the `legacy-dirty-refresh` acknowledgement before writing old state. `rebuild2Cutover.ts` converts legacy large-rebuild rows into V4 rebuild requests, and `inspectDirtyRefreshRisk.ts` labels remaining large-rebuild rows as retired diagnostic state with a V4 rebuild recommendation. Remaining direct script files outside the package command surface are test/debug internals, not supported operator entry points. |

## Admin And Diagnostic Checklist

| Status | Area                        | Files                                                                              | Classification                                         | V4 compatible | R/W               | Uses new CQRS | Legacy | Required handling                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------ | --------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------ | ------------- | ----------------- | ------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [x]    | Admin investigation         | `src/server/routes/AdminInvestigateRoutes.ts`                                      | Admin/debug/tool allowlist and owner diagnostics       | N/A           | Admin/tool        | N/A           | false  | Implemented today as explicit `/api/admin/*` maintenance/settings diagnostics and retired legacy rebuild controls. Evidence: `src/server/routes/routeSurfaceInventory.ts` classifies admin routes as `maintenance-debug-api`, `sensitive-local-api`, or ownerless settings diagnostics; `routeSurfaceInventory.test.ts` proves mounted-route inventory and owner-proxy classification coverage; `AdminInvestigateRoutes.test.ts` covers maintenance and worker runtime diagnostics plus retired legacy large-rebuild mutation endpoints; the retired large-rebuild status route is no longer mounted, and maintenance-runtime diagnostics no longer expose large-rebuild runtime metrics.                                                 |
