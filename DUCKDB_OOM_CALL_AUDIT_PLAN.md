...
- [ ] **A40.** Retire dirty materialization and mart refresh as legacy serving
      ...
      instead of preserving another legacy mart pruning path. A40 remains partial
      because
      `getDuckdbMartMaintenanceService.ts` and large-rebuild workers still retain
      legacy serving writer paths.
...
| [ ]    | Mart refresh worker                     | Deleted `src/server/workers/projectMartRefreshWorker.ts`, `getDuckdbMartMaintenanceService.ts`, deleted `reviewAnswerDictionaryStabilityService.ts`                                                                                | Owner/background writer                                 | Partial       | Writer        | Partial       | true - need further changes | Partial: the worker loop module and its tests are deleted after package scripts, heartbeat loops, and the isolated recovery CLI were removed or rewired to V4 rebuild requests. Current cleanup also deletes the orphaned review-answer dictionary stability service/test instead of preserving an unused legacy mart pruning path. Current remaining legacy mart evidence is direct maintenance service coverage: `getDuckdbMartMaintenanceService.test.ts` asserts `mart.review_article_serving*` state directly instead of routing through V4-only product review-detail hydration. The DuckDB-owner overview route no longer imports this service for legacy mart-refresh throughput, so `getDuckdbMartMaintenanceService.ts` has no non-test callers left. Not checked because the service file/tests still exist and write legacy `mart.review_article_serving*`/rollup structures until the remaining test/setup dependencies are removed.                                                                                                                     |
