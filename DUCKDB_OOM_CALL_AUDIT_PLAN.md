...
- [~] **A18.** Complete review detail/hydration by moving judgment, prompt,
  ...
  payload hydration to keyed V4 detail contracts and fails unavailable when V4
  detail state is missing; remaining blockers are project prompt/config metadata
  and the separate `/api/articles/:id` article-detail history path.
...
- [~] **A30.** Extend admission/diagnostic evidence for unchecked detail,
      ...
      Remaining blockers: warning/health and export still document residual
      source reads or metadata gaps; review detail still has bounded prompt/config
      metadata plus the separate article-detail path; comparison has no separate V4
      admission/contract registry; do not mark those rows migrated yet.
...
- [~] **A34.** Retire `duckdbOlap.ts` raw fallback branches after the last product
  ...
  `duckdbOlap.test.ts` and the article-detail, diagnostic, export, and
  comparison route rows above still list residual blockers; prompt preview and
  normal project review detail no longer contribute raw-fallback blockers.
...
- [ ] **A40.** Retire dirty materialization and mart refresh as legacy serving
      ...
      mart-freshness diagnostics. A40 remains partial because
      `getDuckdbMartMaintenanceService.ts`, `projectMartRefreshWorker.ts`,
      dirty-refresh producers, and large-rebuild workers still retain legacy
      serving paths.
...
- [~] **A50.** Keep review UI article queries gated on V4 warning/readiness data
  ...
  legacy fields do not change product warning copy. Still partial because A51
  warning-query failure handling and final A52 readiness-state wiring remain.
...
| [~]    | Review detail/hydration                    | `src/server/routes/projectsRoutes/projectsRoutesPostArticleReviewDetails.ts`, `src/server/routes/ArticlesRoutes.ts` detail paths                                                                                   | V4 product read plus residual allowlist                 | Partial       | Reader        | Partial       | true - need further changes | Partial: API role proxies/fails closed. Normal project review detail now hydrates the article row, article payload, LLM judgment details, human judgment details, and assessment annotations from keyed V4 detail contracts and returns an explicit unavailable payload when V4 detail state is missing; tests prove it does not call legacy article hydration, app judgment fallback, mart-freshness, Covidence source-record, assessment, model, or snapshot-project fallback reads. Remaining residual reads in `projectsRoutesPostArticleReviewDetails.ts` are bounded project prompt/config metadata, and the separate `ArticlesRoutes.ts` `/api/articles/:id` detail history still reads single-article source/judgment history until rewired or retired.                                                                                                                                                                                                                                                                                                                |
...
| [ ]    | Provider admission/telemetry            | `src/server/cron/judgmentsJobs/providerAdmissionLease.ts`, `src/server/cron/judgmentsJobs/judgmentDispatchTelemetry.ts`, provider repositories                                                                                      | Owner/background writer                                 | Partial       | Reader/writer | Partial       | false                       | Partial: provider admission leases are owner-aware and now use background provider-admission workload contexts for durable telemetry, provider-key reconciliation, and owner-side acquire/heartbeat/release/expire/reconcile transactions; telemetry aggregates owner/judge runtime state in `judgmentDispatchTelemetry.ts` without direct DuckDB access. Provider connection/model repository and route reads now carry owner provider-repository workload contexts, and `providerTelemetryDuckdbAccess.test.ts` guards workload-context markers, owner/internal route classification, and absence of product-review raw scan shapes. Not checked because provider/model write paths still need the A23 V4 dirty/rebuild enqueue proof before this row can be marked complete. |
...
| Status | Area                         | Files                                                      | Classification                                 | V4 compatible | R/W                  | Uses new CQRS | Legacy                      | Required handling                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------ | ---------------------------- | ---------------------------------------------------------- | ---------------------------------------------- | ------------- | -------------------- | ------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [~]    | Review UI freshness/warnings | `src/components/main/reviews/*`, `reviewsWarningsQuery.ts` | Client-over-API plus V4/readiness-consuming UI | Partial       | Client reader        | Partial       | true - need further changes | Partial: no direct DuckDB/client `@duckdb` imports were found in `src/components/main/reviews/*`; `reviewsWarningsQuery.ts` calls `apiClient.api.projectsreviewswarnings.post`, and list containers gate article queries on `warningsQuery.data?.indexing.serving.readable` before calling `articlesreviews*` API query helpers. `routeSurfaceInventory.ts` classifies the review UI API routes as owner-dependent, and `reviewsProjectWarnings.vitest.tsx` now covers stale/unavailable browser copy plus ignoring legacy large-rebuild, dirty-materialization, and quarantine fields in product warning copy. Not complete because the UI still treats warning-query failure as permission to load article rows and final health/warning/detail readiness wiring remains under A51/A52.                                                                                                                                   |
| [~]    | Admin UI                     | `src/app/routes/+admin/*`, `src/components/Navigation.tsx` | Client-over-API admin diagnostics UI / planned | N/A           | Client reader/writer | N/A           | true - need further changes | Partial: no direct DuckDB/client `@duckdb` imports were found in `src/app/routes/+admin/*` or `Navigation.tsx`; admin pages and navigation call Eden/API helpers such as `apiClient.api.admin['duckdb-append-metrics']`, `fetchDuckdbOwnerConnections`, and `fetchLlmStatus`. Current A40/A48/A53 retirement pivot removes the normal admin jobs link to `/admin/project-mart-large-rebuild`, keeps backend large-rebuild status diagnostics, returns retired responses from the legacy large-rebuild mutation endpoints, and removes the stale admin client route plus smoke target so the UI no longer exposes run, pause, resume, or note controls for that path. Not checked because explicit proof that every remaining admin page is diagnostics-only or V4-rewired is incomplete, and dirty-refresh/mart-maintenance admin controls still need separate A48 review. |
