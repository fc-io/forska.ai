...
- [ ] **A40.** Retire dirty materialization and mart refresh as legacy serving
      ...
      acknowledgement before touching old state. A40 remains partial only for
      the dirty-refresh ledger/quarantine state that can disappear after callers
      move fully to V4 delta/rebuild request state.
...
| [~]    | Dirty materialization                   | Deleted `projectMartDirtyMaterializationService.ts`; active legacy ledger/recovery state in `projectMartDirtyRefreshStateService.ts` and quarantine tests/scripts                                                                      | Owner/background writer plus V4 queue/projection state  | Partial       | Writer        | Partial       | true - need further changes | Complete for the old materialization service: whole-project dirty marks write bounded `project_mart_refresh_article_state` rows directly, the direct worker/package entry points are gone, `requestReviewServingForDirtyRefreshClaim.ts` converts dirty-refresh claims into V4 `review_rebuild_request` rows, and `projectMartDirtyMaterializationService.ts` plus its service test are deleted. The remaining package-exposed dirty-refresh commands now use explicit `legacy-*` names, and mutating recovery/quarantine/unquarantine scripts require `legacy-dirty-refresh` admin acknowledgement. Partial only because `projectMartDirtyRefreshStateService.ts` remains as a legacy dirty ledger/quarantine compatibility layer for write fanout, recovery, and stale-state cleanup until those callers move fully to V4 delta/rebuild request state. |
