...
- [~] **A30.** Extend admission/diagnostic evidence for unchecked detail,
      ...
      source reads or metadata gaps; comparison has no separate V4
...
- [~] **A34.** Retire `duckdbOlap.ts` raw fallback branches after the last product
  ...
  `duckdbOlap.test.ts` and the diagnostic, export, and comparison route rows
  above still list residual blockers; prompt preview, normal project review
  detail, and global article-detail history no longer contribute raw-fallback
  blockers.
