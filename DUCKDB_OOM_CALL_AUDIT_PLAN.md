...
| [~]    | Comparison serving builders             | `comparisonProjectServingGenerationService.ts`, `comparisonProjectServingRebuildService.ts`, `comparisonProjectServingRollupBuilder.ts`, `comparisonProjectServingCellBuilder.ts`, `comparisonProjectServingInvalidationService.ts` | Owner/background writer / serving migration             | Partial       | Writer        | Partial       | false                       | Partial serving migration with A42/A21 progress: comparison builders use background DB methods, generation promotion state, and materialize article identity fields plus `mart.comparison_article_identifier_serving` so export and existing-project conflict-resolution import target matching can share bounded serving identities. Generation cleanup deletes the identifier serving table alongside article/cell/filter tables, and rollup/tests prove scoped external IDs, DOI/PMID fields, and identifier rows are projected. `comparisonProjectServingRouteContracts.ts` now supplies the V4-style route/admission registry. Not checked only because broader workload-context proof across comparison route/source surfaces remains open. |
