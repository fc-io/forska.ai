...
- Project-transfer uploads use the normal owner-proxied `/api/*` path with streaming request-body forwarding to the DuckDB owner. Do not add a writer-direct or follower-local upload bypass. `PUT /api/projects/import/:sessionId/upload` must never use the current generic `request.clone().arrayBuffer()` proxy path; it must resolve the owner and fail closed before consuming the request body when no owner is available, stream the original request body, and disable retry for non-replayable upload streams. Small non-transfer API requests may keep the existing buffered retry behavior.
- The streaming upload proxy must branch inside `src/server/routes/ApiProxyRoutes.ts` before any generic proxy helper clones or buffers a non-GET/non-HEAD body; adding route classification alone is not enough because the current proxy template consumes bodies before owner resolution.
- Zip handling uses `@zip.js/zip.js` through a small project-transfer wrapper. Phase 1 must add the dependency and prove streaming read/write, ZIP64 handling, checksum verification, duplicate normalized path rejection, and symlink rejection before route handlers depend on it.
- Import-wizard dependency resolution must not edit existing provider connections in place. Users may select an existing connection, create a new connection, complete provider auth, or leave through the normal provider UI with a session return path. Provider connection reads and updates must preserve the full persisted connection record, including archived state, disabled model ids, manual worker settings, secret refs, saved runtime config, and `maxInflightRequests`; any import-wizard path that edits an existing connection is blocked until the provider by-id read, list payload, and `PATCH /api/provider-connections/:id` route have parity tests proving those fields round-trip exactly.
...
- These decisions define the completed project-transfer behavior.
- The active projects grid exposes `Export Project` immediately next to the existing `Export data` action. Rename `src/components/main/ProjectsGrid.tsx` to `src/components/main/projectsGrid.tsx` with a Git-safe temporary filename step while touching it so new work follows the repo's camelCase filename rule, then update imports and mocks in the same change.
- The archived projects table at `src/app/routes/+projects/+archived/archivedProjectsTable.tsx` exposes the same full-fidelity `Export Project` action so archived projects can be transferred without temporarily unarchiving them.
- The projects index at `src/app/routes/+projects/+index.tsx` exposes `Import Project` immediately to the left of `Create Covidence Project`.
- `Export data` remains mapped to the current `/projects/$id/export` CSV page and active-project CSV semantics.
- `Import Project` opens a dedicated wizard route at `/projects/import` because the import flow needs multiple review and setup steps.
- `Export Project` starts package export from the projects list action: small exports download directly, and large exports create an export session that the active grid or archived table polls until the package is ready. Do not add an intermediate export page.
...
The package contract consists of these manifest-declared root payloads and asset paths. Empty collections still produce valid empty JSON arrays or empty NDJSON files so manifest row counts, checksums, and import validation stay deterministic.
...
- `schemaVersion`; the importer accepts only explicitly supported package schema versions and fails before payload extraction or any database write when the version is unsupported. Package schema versions are file-format compatibility labels, not product-release names.
- `exportedAt`
- `sourceAppVersion`
...
- `warnings[]` entries use a stable object contract: `code`, `severity`, `scope`, `action`, `message`, optional `payload`, optional `jsonPointer`, optional `sourceRef`, and optional `details`.
- `code` is a stable machine-readable identifier used for package fingerprinting and regression tests. Do not derive it from display copy.
- `severity` is one of `info`, `warning`, `fidelity`, or `blocking`. Fatal schema, checksum, zip, and path-safety failures may abort before a manifest warning exists.
- `scope` identifies the affected package area, such as `manifest`, `project`, `providerConnection`, `model`, `prompt`, `article`, `asset`, `judgment`, `humanJudgment`, `humanJudgmentSummary`, `review`, `route`, or `package`.
- `action` identifies what export or import did, such as `sanitized`, `omitted`, `rewritten`, `requiresResolution`, `blocked`, or `provenanceOnly`.
- `payload` names the package payload file when a warning is tied to one file. `jsonPointer` names the field or row location after package serialization and redaction. `sourceRef` may include stable source-provenance labels needed for user review, but package fingerprinting must ignore raw source database ids inside `sourceRef` unless they are the only stable logical identifier for that warning class.
- `details` is optional structured data for counts, field names, candidate identifiers, or dependency descriptors. It must not contain secrets, source-machine paths, raw credentials, token-like URL data, or volatile session values.
...
- With the current schema, `app.judgment_execution_snapshot` is not durable proof for an exported `app.judgment` by itself because it has no `judgment_id` or result-output linkage. The transfer flow must treat snapshots as unusable for judgment provenance unless implementation adds and tests an explicit unique linkage algorithm; otherwise use `currentReviewRows` provenance for non-chunked rows and omit chunked rows that lack a stored/future signature.
...
- Article full-text conversion runtime state in this transfer flow: `full_text_conversion_status`, `full_text_conversion_error`, `full_text_conversion_attempts`, `full_text_conversion_model_id`, and `full_text_conversion_metadata`; surface this as an omission warning because it is machine-local pipeline state, not portable package content.
...
- Reuse the existing provider setup flows during import instead of inventing a separate import-only credential path, but do not patch an existing chosen provider connection through a path that drops persisted values such as archived state, disabled model ids, manual worker settings, secret refs, or saved `maxInflightRequests`. The import wizard forbids in-wizard edits of existing provider connections. Any existing-connection edit path must preserve the full existing config, change only reviewed fields, and have by-id repository read plus generic PATCH parity tests proving those fields round-trip exactly before the edit path is enabled.
...
- Use the normal provider endpoints for dependency setup where possible: `GET /api/provider-connections` for listing and refresh, `POST /api/provider-connections` for connection creation, `POST /api/provider-auth/:providerKind/begin`, `POST /api/provider-auth/:providerKind/finish`, and `POST /api/provider-connections/:id/test` for connection setup and verification, `GET /api/provider-connections/:id/discovered-models` for candidate review, `POST /api/provider-connections/:id/models` and `POST /api/provider-connections/:id/sync-models` for non-Codex model materialization, and the Codex runtime endpoints `GET /api/models/codex/status`, `POST /api/models/codex/login`, `GET /api/models/codex/login/:jobId`, plus `POST /api/models/ensure` for Codex materialization. Import does not call `PATCH /api/provider-connections/:id` for existing provider connections.
...
- The current `GET /api/provider-connections` list intentionally exposes only visible/non-archived connection choices, so dependency mapping cannot rely on it to inspect archived target connections. Treat an archived target connection as unavailable for automatic mapping unless a separate audited endpoint is added later.
...
- Import always creates a new `app.project` row; do not overwrite an existing project.
...
- Import never merges into an existing project; each successful import creates a new project.
...
- Analyze must also compare planned reused-article `articleCreatedAt` fills against every existing target project, active or archived, that already references those target article ids through `app.project_article` or linked import routes. If filling a previously missing target article date would make the article newly enter an unrelated existing project's current date-bounded scope, block unless import can skip that fill without breaking imported-project scope and `judgmentInputSignature` validation; the transfer flow must not silently expand unrelated existing projects through shared article row updates.
- For reused target articles with imported judgments, analyze and commit must recompute each `judgmentInputSignature` after planned article field fills, asset-promotion path rewrites, prompt remapping, and model/provider mapping. If source and target signatures differ because the target already has non-empty prompt-input content that import would not overwrite, block as a judgment fidelity conflict; the transfer flow does not silently import those judgments against different article content.
- For reused target articles with imported human judgments, human summary judgments, or review rows, analyze and commit must recompute each `humanReviewInputSignature` after planned article field fills, asset rewrites, and prompt remapping. If the target article or target prompt already has non-empty certified content that import would not overwrite and the recomputed signature differs, block as a human/review fidelity conflict; the transfer flow must not silently import durable human assessment or reviewed-section state against different article or prompt content.
- If the non-destructive merge updates any reused article row, mark every active project currently referencing those updated articles dirty inside the same transaction, in addition to marking the new imported project dirty. For archived referencing projects, analyze must still report current and future expansion risks. The import flow does not write archived dirty rows during import; instead it must patch and test the unarchive path to derive dirty article ids from current app tables before queueing refresh, so archived projects rebuild from the imported or merged article state when later unarchived.
...
- Title, year, author, and source-metadata heuristics may help the review UI explain likely matches, but they must not silently auto-link an article.

## Judgment Integrity Rules

- Export and import only answered active current-review judgments where `deleted_at IS NULL` and `is_answered = TRUE`.
- Import judgments only after article, prompt, and model mappings are fully resolved to identity-equivalent target rows; a user-selected non-equivalent substitute model must not be used to label imported judgment rows or to avoid uniqueness conflicts.
...
- Import validation must preserve and display `humanReviewInputSignatureProvenance`. Human/review rows with `currentReviewRows` provenance may import only when the target-remapped `humanReviewInputSignature` matches exactly; rows with future stored/snapshot provenance must also match the target-remapped signature and must show that stronger provenance in the plan. Mismatches are blocking human/review fidelity conflicts, not warnings, because the current app cannot safely rewrite answered human decisions or reviewed-section state to a different article/prompt payload.
- Preserve answer payloads, explanation, quotes, chunking strategy, and timestamps where safe.
- `confidenceOriginal` must be normalized before insert because the DuckDB `app.judgment.confidence_original` column is `NOT NULL DEFAULT 50` while current TypeScript record shapes may allow `null`. Import must either preserve a concrete exported confidence value or write the DB-equivalent default `50`; it must never insert `null`.
...
- Source `app.judgment_human.project_id` is nullable in the current DB schema, but transfer imports must still write the new target project id for every imported human judgment row and must not depend on source non-nullness.
...
- Give each import session a TTL plus cleanup on commit, cancel, expiry, and best-effort startup recovery. Do not add product-level hard package-size caps; fail only when local machine resources are insufficient for the requested import.
...
4. Review import plan.
   ...
   - Show any blocking package-contract conflicts, article-match conflicts, reused-article date-scope conflicts for the imported project or unrelated existing projects, project-prompt remap conflicts, post-remap judgment conflicts, or human/review fidelity conflicts. These are review-time blockers, not an in-wizard per-row remap tool.
...
- Project-transfer upload routes stay owner-proxied. Do not classify upload routes as follower-local or writer-direct bypass routes. The streaming upload proxy path must check for a DuckDB owner before consuming the request stream, fail closed without an owner, must not buffer the upload body into an `ArrayBuffer`, and must not retry after a partially streamed upload body could have reached the owner. Add a no-owner regression test that proves the upload request body is not consumed when no owner is available.
...
- These project-transfer endpoints stay on the normal owner-proxied `/api/*` path. Any follower-local polling or download exception requires explicit `apiRouteClassification.ts` changes.
...
- `GET /api/projects/export/:exportId/download`
  ...
  - must stream through the existing owner-proxy response path without materializing the package in follower memory; returning the owner `fetch` response directly is likely sufficient, but keep an explicit regression test because upload proxying needs a separate streaming branch
...
- [ ] Do not edit existing provider connections in the import wizard; if dependency setup leaves the wizard for standalone provider pages, add a `returnTo` or import-session handoff contract first.
...
- Recommended first ralph loop: convert only `Ralph User Stories For First Loop` into a `prd.json` focused on Phase 1 foundations.
- Conversion metadata: use `name: "Project Transfer Foundation"`, `branchName: "ralph/project-transfer-foundation"`, and `description: "Add the safe project-transfer foundation: schema, route ordering, session/history repositories, upload proxying, zip/path safety, manifest contracts, recovery, and runtime asset hardening."`
- Conversion rule: each `US-*` below maps to one `userStories[]` entry with `passes: false`, `notes: ""`, and the listed `dependsOn` values. Do not convert phase checklists, risk lists, or done criteria directly into separate stories.
- Acceptance criteria rule: when creating `prd.json`, combine each story's `Acceptance criteria` and `Quality gates` into that story's `acceptanceCriteria[]` array.
- Quality-gate rule: keep gates story-local. Use targeted `bun test <file>` or `bunx vitest run <file>` whenever possible; add `bun run db:mig` for schema stories, `bun run build` for route/API/client contract stories, and `bun run desktop:build` only for shared runtime, upload/download, asset-path, or desktop-relevant behavior. Use `bun run lint` for touched `src` layers without fixing unrelated lint issues.
...
- `Zip rules`
  - Use `@zip.js/zip.js` for package read/write through a small server wrapper. Acceptance tests must prove streaming package creation, streaming extraction, ZIP64 archives over 4 GB or a deterministic ZIP64 fixture, byte-for-byte checksum validation, duplicate normalized path rejection, and symlink rejection before export or import route handlers rely on the wrapper.
...
- `Writer ownership`
  ...
  - Readonly session polling must stay on the owner-proxied `/api/*` path. Follower-local polling is only safe if `apiRouteClassification.ts` is explicitly updated for those paths, and follower servers must never mutate transfer sessions or start background transfer work.
...
- `Import session states`
  ...
  - `awaiting_resolution` means the package is parsed but still has provider or model dependencies to resolve, or it has blocking package-settings, article, project-prompt, judgment, or human/review fidelity conflicts that prevent commit. Article, project-prompt, judgment, and human/review fidelity conflicts are not remapped in-wizard; expose `canCommit: false` plus a resolution kind such as `requires_new_package_or_target_changes` until the package or target data changes. Missing or inactive import routes stay warnings and do not keep the session unresolved on their own.
...
- `Duplicate history store`
  ...
  - Where DuckDB cannot express conditional completed-import-only constraints cleanly, enforce the invariant in the repository transaction that writes completion history and add tests for invalid incomplete rows in commit retry/recovery paths.
...
- `Provider/model resolution contract`
  ...
  - If imported judgments depend on a source provider endpoint that has no portable endpoint identity after redaction, do not auto-match it from label, machine-local URL, or worker URL. Keep the dependency unresolved until the user maps to an identity-equivalent target connection/model that still reproduces the `judgmentInputSignature`, or omit/block the affected judgments according to the project-transfer fidelity rules.
  ...
  - If an exported model lacks `remoteModelId`, never assume it can be materialized through the current non-Codex create route. Only auto-match an existing enabled model when fallback identity fields from the package, such as `modelName`, `name`, `displayName`, `variant`, and `version`, resolve to one row on the mapped connection; otherwise keep the model unresolved. The import flow may allow an explicitly acknowledged non-equivalent substitute only for a project model that has no imported judgment rows; any source model referenced by imported judgments must resolve through exact fallback identity or the session stays blocked.
...
- `Phase 1 tests` owner files: new `src/server/services/projectTransfer/projectTransferManifest.test.ts`, new `src/server/services/projectTransfer/projectTransferPaths.test.ts`, new `src/server/services/projectTransfer/projectTransferSessionRepository.test.ts`, new `src/server/services/projectTransfer/projectTransferSessionRecovery.test.ts`, new `src/server/services/projectTransfer/projectTransferHistoryRepository.test.ts`, new `src/server/routes/projectTransferRoutes.test.ts`, new `src/server/routes/apiRouteClassification.test.ts` if transfer routes change classification behavior, and existing `src/server/routes/ApiProxyRoutes.test.ts` plus `src/server/routes/ApiProxyRoutes.retry.test.ts` for streaming upload proxy behavior. Cover manifest validation, fingerprint stability, zip/path rejection, runtime asset path rejection, route-level contract failures, route shadowing, durable session compare-and-set transitions, writer-only recovery/cleanup, duplicate-history invariants, and the streaming owner-proxy behavior for transfer uploads.
  Include a route-shadowing regression for the existing CSV `POST /api/projects/:id/export` route even if no focused CSV route test exists today, so the new `/api/projects/:id/export-project`, `/api/projects/export/:exportId`, and `/api/projects/import/...` routes cannot accidentally claim or break it.
...
- [ ] Cover no-owner project-transfer upload proxy failure without consuming the upload request body
...
- [ ] Add focused coverage that the existing CSV `POST /api/projects/:id/export` route still resolves to the CSV export flow after adding `POST /api/projects/:id/export-project` and `/api/projects/export/:exportId` transfer routes, because no dedicated CSV route-shadowing test exists today
...
- [ ] Add provider resolution coverage proving existing-connection edits are forbidden in the import wizard and users must select an existing connection, create a new connection, or return through the normal provider UI using list payload state
...
- [ ] Implement final-path asset promotion plus the final transaction that creates the new active project, immutable prompt rows or links, article rows and links, new judgment rows, human judgments, human summary judgments, reviews, and assessments with remapped ids, plus mart refresh dirty state and the completed transfer-history row. Equivalent reused judgments are not written as new final app-table references; record their source-to-target mappings and provenance in the frozen import plan, completion payload, and transfer history.
...
- [ ] Implement a dedicated project-transfer judgment writer instead of the shared append path. It must explicitly write `is_answered = TRUE`, `delete_generation`, `deleted_at = NULL` for answered-active-only transfer imports, `snapshot_project_id`, exported `snapshot_project_model_name` exactly, timestamps, imported answer fields, and a non-null `confidence_original` for new judgment rows, skip inserts only for judgments preclassified by analyze as equivalent reusable target rows, keep reused target snapshot labels unchanged, and fail on every other physical DuckDB uniqueness conflict or active review-visible natural-key conflict rather than using `ON CONFLICT ... DO NOTHING`.
...
- [ ] Cover imported judgments with missing or null package confidence normalize to the DB-equivalent default instead of inserting `NULL`, and imported human judgments always write the new target project id even when the source row had nullable project provenance
...
- Article matching stays exact-identifier-only; heuristics can explain likely matches in review but cannot silently auto-link.
- Immutable prompt reuse means cross-project judgment collisions can still happen after article, prompt, and model remap, so conflict detection must stay strict across both the physical DuckDB key that includes `delete_generation` and the active review-visible natural key that excludes it; only explicitly equivalent physical-key target judgments may be reused.
- Imported prompt links and imported project-article links will leave `origin_project_id` and `imported_from_project_id` null. Source-package provenance lives in the manifest, import session summary, transfer history, and post-import warnings instead.
...
- Large upload support depends on the writer-proxy path. Project transfer uses streaming owner proxying for project-transfer uploads; if `ApiProxyRoutes.ts` still buffers `PUT /api/projects/import/:sessionId/upload` into an `ArrayBuffer`, Phase 3 is blocked.
...
- Missing or inactive import routes and target routes that would expand project scope will be omitted with warning, and exported route-scoped articles will be preserved through direct snapshot project-article links.
